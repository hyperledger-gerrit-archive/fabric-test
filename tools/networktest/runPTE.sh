...
usage () {
    ...
    echo -e "--dir\t\trelative path from .../fabric-test/tools/networktest/ to the directory that contains the network connection profiles to be converted"
    ...
    echo -e "--org <list of 2 org names>"
    echo -e "\t\t(Default: org1 org2)"
    echo
...
testPreProc() {
    ...
    # channel
    echo -e "[testPreProc] replace testorgschannel1 with $Channel"
    if [ -e CITest/$tcase/preconfig ]; then
        sed -i "s/testorgschannel1/$Channel/g" CITest/$tcase/preconfig/channels/*
        sed -i "s/testorgschannel1/$Channel/g" CITest/$tcase/preconfig/$tcc/*
    fi
    sed -i "s/testorgschannel1/$Channel/g" CITest/$tcase/$tcc/*

    # org1
    echo -e "[testPreProc] replace org1 with ${Organization[0]}"
    if [ -e CITest/$tcase/preconfig ]; then
        sed -i "s/org1/${Organization[0]}/g" CITest/$tcase/preconfig/channels/*
        sed -i "s/org1/${Organization[0]}/g" CITest/$tcase/preconfig/$tcc/*
    fi
    sed -i "s/org1/${Organization[0]}/g" CITest/$tcase/$tcc/*

    # org2
    echo -e "[testPreProc] replace org2 with ${Organization[1]}"
    if [ -e CITest/$tcase/preconfig ]; then
        sed -i "s/org2/${Organization[1]}/g" CITest/$tcase/preconfig/channels/*
        sed -i "s/org2/${Organization[1]}/g" CITest/$tcase/preconfig/$tcc/*
    fi
    sed -i "s/org2/${Organization[1]}/g" CITest/$tcase/$tcc/*
