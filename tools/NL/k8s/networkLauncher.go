...
func (NL *NetworkLauncher) CreatePvcs(kubeConfigPath string) error {
	...
	err := NL.ExecuteCommand("kubectl", []string{fmt.Sprintf("--kubeconfig=%v", kubeConfigPath), "apply", "-f", "./configFiles/fabric-pvc.yaml"})
	if err != nil {
		return err
	}
	return nil
...
func (NL *NetworkLauncher) NetworkCleanUp(networkSpec Config, kubeConfigPath string) error {
	...
	err = os.RemoveAll(path)
	path = filepath.Join(currentDir, networkSpec.ArtifactsLocation, "channel-artifacts")
	err = os.RemoveAll(path)
	path = filepath.Join(currentDir, networkSpec.ArtifactsLocation, "crypto-config")
	err = os.RemoveAll(path)
	if err != nil {
		return err
	}
