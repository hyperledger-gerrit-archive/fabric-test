...
func NetworkCleanUp(networkSpec Config, kubeConfigPath string) error {
	...
	err = os.RemoveAll(path)
	path = filepath.Join(networkSpec.ArtifactsLocation, "connection-profiles")
	err = os.RemoveAll(path)
	if networkSpec.K8s.DataPersistence == "local" {
...
func deleteSecrets(numComponents int, componentType, orgName, kubeConfigPath, tls string) {
	...
	if (componentType == "peer" || componentType == "orderer") && tls == "mutual" {
		err := ExecuteK8sCommand(kubeConfigPath, "delete", "secrets", fmt.Sprintf("%v-clientrootca-secret", orgName))
		if err != nil {
			fmt.Println(err.Error())
		}
	}
