...
func generateGenesisBlockNChannelTransaction(networkSpec NL.Config, kubeConfigPath string) error {
    ...
    for i := 0; i < networkSpec.NumChannels; i++ {
        err := NL.ExecuteCommand("configtxgen", "-profile", "testorgschannel", "-channelID", fmt.Sprintf("testorgschannel%v", i), "-outputCreateChannelTx", fmt.Sprintf("%v/testorgschannel%v.tx", path, i), "-configPath=./configFiles/")
        if err != nil {
            return err
        }
        for j := 0; j < len(networkSpec.PeerOrganizations); j++ {
			err := NL.ExecuteCommand("configtxgen", "-profile", "testorgschannel", "-outputAnchorPeersUpdate", fmt.Sprintf("%v/%vanchor.tx", path, networkSpec.PeerOrganizations[j].MSPID), "-asOrg", fmt.Sprintf("%v", networkSpec.PeerOrganizations[j].Name), "-channelID", fmt.Sprintf("testorgschannel%v", i), "-configPath=./configFiles/")
			if err != nil {
				return err
			}
		}
