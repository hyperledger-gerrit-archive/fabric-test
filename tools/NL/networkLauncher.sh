...
# yaml parser
function yamlParser {
    echo "[yamlParser] network yaml input: $yamlF"
    ...
    nChannel=`yq r $yamlF num_of_channels`
    echo "[yamlParser] nChannel=$nChannel"

    #organisations
    tmp=`yq r $yamlF organizations[*].name`
    ...
    while [ $i -gt 0 ]
    do
        t1=`echo $tmp | cut -f $i -d ' '`
        if [ "$t1" == "" ]; then
            break
        fi
        if [ "$t1" == "ordererorg" ]; then
            ordererOrgName[$oi]=`yq r $yamlF organizations[$j].name`
            ordererOrgMspid[$oi]=`yq r $yamlF organizations[$j].msp_id`
            ordererOrgnOrderers[$oi]=`yq r $yamlF organizations[$j].num_of_orderers`
            let oi=oi+1
            let j=j+1
        elif [ "$t1" != "-" ]; then
            peerOrgName[$pi]=`yq r $yamlF organizations[$j].name`
            peerOrgMspid[$pi]=`yq r $yamlF organizations[$j].msp_id`
            peerOrgnCAs[$pi]=`yq r $yamlF organizations[$j].num_of_ca`
            peerOrgnPeers[$pi]=`yq r $yamlF organizations[$j].num_of_peers`
            let pi=pi+1
            let j=j+1
        fi
        let i=i+1
    done
    echo "[yamlParser] orderer org name: ${ordererOrgName[@]}"
    echo "[yamlParser] orderer org msp_id: ${ordererOrgMspid[@]}"
    echo "[yamlParser] orderer org nOrderers: ${ordererOrgnOrderers[@]}"

    echo "[yamlParser] peer org name: ${peerOrgName[@]}"
    echo "[yamlParser] peer org msp_id: ${peerOrgMspid[@]}"
    echo "[yamlParser] peer org nCA: ${peerOrgnCAs[@]}"
    echo "[yamlParser] peer org nPeers: ${peerOrgnPeers[@]}"
