...
async function genConfigtx(networkSpec) {
    ...
    try {
        ...
        for (let i = 0; i < networkSpec.num_channels; i++) {
            await exec('configtxgen -profile testorgschannel -channelCreateTxBaseProfile testOrgsOrdererGenesis -channelID testorgschannel'+i+' -outputCreateChannelTx ./testorgschannel'+i+'.tx')
        }
    } catch (err) {
        //TODO
    }
