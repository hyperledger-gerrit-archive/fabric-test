...
func (n *NetworkLauncher) GenerateCryptoCerts(networkSpec Config) error {
	...
	cmd := exec.Command("cryptogen", "generate", "--config=./configFiles/crypto-config.yaml", fmt.Sprintf("--output=%", configPath))
	stdoutStderr, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("%v", err)
	}
...
func init() {
	...
	path := filepath.Join(currentDir, "ytt-linux-amd64")
	if _, err = os.Stat(path); os.IsNotExist(err) {
		cmd := exec.Command("wget", "https://github.com/k14s/ytt/releases/download/v0.11.0/ytt-linux-amd64")
		_, err := cmd.CombinedOutput()
		if err != nil {
			log.Fatalf("%v", err)
		}
		cmd = exec.Command("chmod", "+x", "ytt-linux-amd64")
		_, err = cmd.CombinedOutput()
		if err != nil {
			log.Fatalf("%v", err)
		}
