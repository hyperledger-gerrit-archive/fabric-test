...
func (n *NetworkLauncher) GenerateConfigurationFiles() error {
	...
	cmd := exec.Command("./ytt-darwin-amd64", "-f", "./templates/", "--output", "./configFiles")
...
func (n *NetworkLauncher) GenerateOrdererGenesisBlock(kubeConfigPath string) error {
	cmd := exec.Command("configtxgen", "-profile", "testOrgsOrdererGenesis", "-channelID", "ordersystemchannel", "-outputBlock", "./genesis.block", "-configPath=./configFiles/")
	stdoutStderr, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("%v", err)
	}
	fmt.Printf(string(stdoutStderr))
	cmd = exec.Command("kubectl", fmt.Sprintf("--kubeconfig=%v", kubeConfigPath), "create", "secret", "generic", "genesisblock", "--from-file=./genesis.block")
	stdoutStderr, err = cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("%v", err)
	}
...
func (n *NetworkLauncher) GenerateChannelTransaction(networkSpec Config) error {
	...
	for i := 0; i < networkSpec.NumChannels; i++ {
		cmd := exec.Command("configtxgen", "-profile", "testorgschannel", "-channelCreateTxBaseProfile", "testOrgsOrdererGenesis", "-channelID", fmt.Sprintf("testorgschannel%v", i), "-outputCreateChannelTx", fmt.Sprintf("./testorgschannel%v.tx", i), "-configPath=./configFiles/")
		stdoutStderr, err := cmd.CombinedOutput()
		if err != nil {
			return fmt.Errorf("%v", err)
		}
...
func init() {
	...
	path := filepath.Join(currentDir, "ytt-darwin-amd64")
	if _, err = os.Stat(path); os.IsNotExist(err) {
		cmd := exec.Command("wget", "https://github.com/k14s/ytt/releases/download/v0.11.0/ytt-darwin-amd64")
		_, err := cmd.CombinedOutput()
		if err != nil {
			log.Fatalf("%v", err)
		}
		cmd = exec.Command("chmod", "+x", "ytt-darwin-amd64")
		_, err = cmd.CombinedOutput()
		if err != nil {
			log.Fatalf("%v", err)
		}
...
func main() {
	...
	err = NL.GenerateOrdererGenesisBlock(*kubeConfigPath)
	if err != nil {
