...
func (n *NetworkLauncher) GenerateConfigFiles(input interface{}) error {
	...
	if err != nil {
		return errors.New(fmt.Sprintf("Failed to create %v path", action))
	}

	defer config.Close()
	template, err := template.New("configtx").Parse(fileTemplate)
	if err != nil {
		return errors.New(fmt.Sprintf("Failed to parse %v template", action))
	}

	err = template.Execute(io.MultiWriter(config), input)
	if err != nil {
		return errors.New(fmt.Sprintf("Failed to create %v.yaml", action))
	}
