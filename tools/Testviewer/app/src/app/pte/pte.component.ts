...
export class PTEComponent implements OnInit {
  ...
  tests = {};
  test_toadd = ['FAB-3807-4i','FAB-3808-2i','FAB-3832-4i','FAB-3833-2i'];
  options = 0;
  buildnum;
  private chosendate;
  selectedOptions;
  ...
  getData() {
    ...
    */
  	for (let fabnum of this.selectedOptions) {
  		fetch(`${serverurl}/pte/${fabnum}/${this.chosendate}` ,{
	       method:'GET',
	     })
       .then(res => res.json())
       .then(res => {
  ...
  loadAll(startdate, enddate) {
    ...
    this.dateselectService.checkEndDate(`${serverurl}/pte/${this.test_toadd[0]}/${this.dateselectService.convertDateFormat(enddate)}`)
    .then(bool => {
      if (bool == true) {
  ...
  ngOnInit() {
    ...
    this.selectedOptions = ['FAB-3807-4i','FAB-3808-2i','FAB-3832-4i','FAB-3833-2i']
