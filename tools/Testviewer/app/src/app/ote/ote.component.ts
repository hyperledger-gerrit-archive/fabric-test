...
export class OTEComponent implements OnInit {
  ...
  tests = {}
  test_toadd = ['FAB-6996','FAB-7036','FAB-7038','FAB-7060','FAB-7061','FAB-7080','FAB-7081']
  options = 0;
  buildnum;
  private chosendate;
  selectedOptions;
  ...
  getData() {
    /*
    Fetches data of all tests
    */
  	for (let fabnum of this.selectedOptions) {
  		fetch(`${serverurl}/ote/${fabnum}/${this.chosendate}` ,{
	       method:'GET',
	     })
       .then(res => res.json())
       .then(res => {
  ...
  loadAll(startdate, enddate) {
    ...
    this.dateselectService.checkEndDate(`${serverurl}/ote/${this.test_toadd[0]}/${this.dateselectService.convertDateFormat(enddate)}`)
    .then(bool => {
      if (bool == true) {
  ...
  ngOnInit() {
    ...
    this.selectedOptions = ['FAB-6996','FAB-7036','FAB-7038','FAB-7060','FAB-7061','FAB-7080','FAB-7081']
