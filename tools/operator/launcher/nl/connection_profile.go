	...
	for org := 0; org < numOrdererOrganizations; org++ {
		...
		for i := 0; i < numOrderers; i++ {
			...
			if input.K8s.ServiceType == "NodePort" {
				...
				k8sNodeIP = getK8sExternalIP(kubeconfigPath, input, "")
			} else {
				portNumber = "7050"
				k8sNodeIP = getK8sExternalIP(kubeconfigPath, input, ordererName)
			}
			protocol = "grpc"
			if input.TLS == "true" || input.TLS == "mutual" {
				protocol = "grpcs"
			}
			orderer = networkspec.Orderer{MSPID: ordererOrg.MSPID, URL: fmt.Sprintf("%v://%v:%v", protocol, k8sNodeIP, portNumber), AdminPath: filepath.Join(input.ArtifactsLocation, fmt.Sprintf("/crypto-config/ordererOrganizations/%v/users/Admin@%v/msp", ordererOrg.Name, ordererOrg.Name))}
...
func certificateAuthorities(peerOrg networkspec.PeerOrganizations, kubeconfigPath string, input networkspec.Config) map[string]networkspec.CertificateAuthority {
	...
	for i := 0; i < peerOrg.NumCA; i++ {
		...
		if input.K8s.ServiceType == "NodePort" {
			...
			portNumber = "7054"
			k8sNodeIP = getK8sExternalIP(kubeconfigPath, input, caName)
		}
		protocol = "http"
		if input.TLS == "true" || input.TLS == "mutual" {
			protocol = "https"
		}
		CA = networkspec.CertificateAuthority{URL: fmt.Sprintf("%v://%v:%v", protocol, k8sNodeIP, portNumber), CAName: caName}
	...
	for org := 0; org < len(input.PeerOrganizations); org++ {
		...
		for i := 0; i < input.PeerOrganizations[org].NumPeers; i++ {
			...
			if input.K8s.ServiceType == "NodePort" {
				...
				portNumber = "7051"
				k8sNodeIP = getK8sExternalIP(kubeconfigPath, input, peerName)
			}
			protocol = "grpc"
			if input.TLS == "true" || input.TLS == "mutual" {
				protocol = "grpcs"
			}
			peer = networkspec.Peer{URL: fmt.Sprintf("%v://%v:%v", protocol, k8sNodeIP, portNumber)}
