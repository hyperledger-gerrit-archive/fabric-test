...
func ordererOrganizations(networkSpec helper.Config, kubeconfigPath string) map[string]helper.Orderer {
	orderers := make(map[string]helper.Orderer)
...
func certificateAuthorities(peerOrg helper.PeerOrganizations, kubeconfigPath string, networkSpec helper.Config) map[string]helper.CertificateAuthority {
	CAs := make(map[string]helper.CertificateAuthority)
...
func peerOrganizations(networkSpec helper.Config, kubeconfigPath string) error {

	for org := 0; org < len(networkSpec.PeerOrganizations); org++ {
		peers := make(map[string]helper.Peer)
		organizations := make(map[string]helper.Organization)
		peerorg := networkSpec.PeerOrganizations[org]
		var peer helper.Peer
		var organization helper.Organization
		peersList := []string{}
		for i := 0; i < networkSpec.PeerOrganizations[org].NumPeers; i++ {
			...
			peers[peerName] = peer
			organization = helper.Organization{Name: peerorg.Name, MSPID: peerorg.MSPID}
		}
...
func generateConnectionProfileFile(kubeconfigPath, orgName string, networkSpec helper.Config, peerOrganizations map[string]helper.Peer, organizations map[string]helper.Organization, certificateAuthorities map[string]helper.CertificateAuthority) error {
	...
	cp := helper.ConnectionProfile{Client: client, Channels: channels, Organizations: organizations, Orderers: orderersMap, Peers: peerOrganizations, CA: certificateAuthorities}
...
func CreateConnectionProfile(networkSpec helper.Config, kubeconfigPath string) error {
