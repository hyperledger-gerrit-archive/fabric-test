...
func createCertsConfigmap(numComponents int, numCA int, componentType, orgName, kubeConfigPath string, input networkspec.Config) {
	...
	for j := 0; j < numComponents; j++ {
		...
		err := client.ExecuteK8sCommand(kubeConfigPath, "create", "configmap", fmt.Sprintf("%s-msp", componentName), fmt.Sprintf("--from-file=admincerts=%s/msp/admincerts/", path), fmt.Sprintf("--from-file=cacerts=%s/msp/cacerts/", path), fmt.Sprintf("--from-file=signcerts=%s/msp/signcerts/", path), fmt.Sprintf("--from-file=keystore=%s/msp/keystore/", path), fmt.Sprintf("--from-file=tlscacerts=%s/msp/tlscacerts/", path))
		if err != nil {
...
func createCaCertsConfigmap(componentType, orgName, kubeConfigPath string, input networkspec.Config) {
	componentName := fmt.Sprintf("ca-%s", orgName)
