...
func launchMspSecret(numComponents int, isCA bool, componentType, orgName, kubeConfigPath string, input networkspec.Config) error{
	...
	cryptoConfigPath := CryptoConfigDir(input.ArtifactsLocation)
	for j := 0; j < numComponents; j++ {
		componentName = fmt.Sprintf("ca%d-%s", j, orgName)
		if isCA != true {
			componentName = fmt.Sprintf("%s%d-%s", componentType, j, orgName)
			path = JoinPath(cryptoConfigPath, fmt.Sprintf("%sOrganizations/%s/%ss/%s.%s", componentType, orgName, componentType, componentName, orgName))
		}
		caPath = JoinPath(cryptoConfigPath, fmt.Sprintf("%sOrganizations/%s", componentType, orgName))
