...
func CreateMspSecret(networkSpec helper.Config, kubeConfigPath string) {
...
func launchMspSecret(numComponents int, isCA bool, componentType, orgName, kubeConfigPath string, networkSpec helper.Config) {
	...
	if isCA == false && networkSpec.TLS == "mutual" {
		err := Client.ExecuteK8sCommand(kubeConfigPath, "create", "secret", "generic", fmt.Sprintf("%v-clientrootca-secret", orgName), fmt.Sprintf("--from-file=%v/crypto-config/%vOrganizations/%v/ca/ca.%v-cert.pem", networkSpec.ArtifactsLocation, componentType, orgName, orgName))
		if err != nil {
