...
func (n Network) GenerateConfigurationFiles(kubeConfigPath string) error {
...
func (n Network) Generate(input networkspec.Config) error{

	configFilesPath := utils.ConfigFilesDir()
	err := n.GenerateConfigurationFiles()
	if err != nil {
		log.Println("Failed to generate yaml files")
		return err
	}
	err = n.GenerateCryptoCerts(input)
	if err != nil {
		log.Println("Failed to generate certificates")
		return err
	}

	err = n.GenerateGenesisBlock(input)
	if err != nil {
		log.Println("Failed to create orderer genesis block")
		return err
	}

	err = client.GenerateChannelTransaction(input, configFilesPath)
	if err != nil {
		log.Println("Failed to create channel transaction")
		return err
