...
func GetConfigData(networkSpecPath string) (networkspec.Config, error) {
	...
	if err != nil {
		log.Printf("Failed to read input file")
		return config, err
	}
	err = yaml.Unmarshal(yamlFile, &config)
	if err != nil {
		log.Printf("Failed to create config object")
		return config, err
	}
	return config, nil
}
...
func GenerateConfigurationFiles(kubeConfigPath string) error {
	var err error
	configtxPath := TemplateFilePath("configtx")
	cryptoConfigPath := TemplateFilePath("crypto-config")
	inputFilePath := TemplateFilePath("input")
	configFilesPath := fmt.Sprintf("--output=%s", ConfigFilesDir())
	k8sDir, dockerDir := TemplateFilePath("k8s"), TemplateFilePath("docker")
	ytt := YTTPath()
	if kubeConfigPath != "" {
...
func GenerateCryptoCerts(input networkspec.Config, kubeConfigPath string) error {
	...
	configPath := CryptoConfigDir(input.ArtifactsLocation)
	config := fmt.Sprintf("--config=%s", JoinPath(ConfigFilesDir(), "crypto-config.yaml"))
...
func GenerateGenesisBlock(input networkspec.Config, kubeConfigPath string) error {
	...
	path := ChannelArtifactsDir(input.ArtifactsLocation)
...
func changeKeyName(artifactsLocation, orgType, orgName, caType string, numCA int) error {
	...
	path := JoinPath(artifactsLocation, fmt.Sprintf("crypto-config/%sOrganizations/%s/%s", orgType, orgName, caType))
	for j := 0; j < numCA; j++ {
		files, err := ioutil.ReadDir(path)
		if err != nil {
			log.Printf("Failed to read files")
			return err
		}
		for _, file := range files {
			if strings.HasSuffix(file.Name(), "_sk") && file.Name() != "priv_sk" {
				err = client.ExecuteCommand("cp", JoinPath(path, file.Name()), JoinPath(path, "priv_sk"))
				if err != nil {
					log.Printf("Failed to copy files")
					return err
				}
			}
		}
