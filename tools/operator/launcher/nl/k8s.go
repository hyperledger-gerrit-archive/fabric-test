...
func LaunchK8sComponents(kubeConfigPath string, isDataPersistence string) error {
	...
	k8sServicesFile := helper.ConfigFilePath("k8s")
...
func DownK8sComponents(kubeConfigPath string, input networkspec.Config) error {
	...
	for i := 0; i < numOrdererOrganizations; i++ {
		...
		if err != nil{
			log.Println("Failed to delete orderer secrets in %s", ordererOrg.Name)
		}
		err = deleteSecrets(ordererOrg.NumCA, "ca", ordererOrg.Name, kubeConfigPath, input.TLS)
		if err != nil{
			log.Println("Failed to delete ca secrets in %s", ordererOrg.Name)
		}
	}
	...
	for i := 0; i < len(input.PeerOrganizations); i++ {
		...
		if err != nil{
			log.Println("Failed to delete peer secrets in %s", peerOrg.Name)
		}
		deleteSecrets(peerOrg.NumCA, "ca", peerOrg.Name, kubeConfigPath, input.TLS)
		if err != nil{
			log.Println("Failed to delete ca secrets in %s", peerOrg.Name)
		}
	}
	k8sServicesFile := helper.ConfigFilePath("k8s")
	k8sPodsFile := helper.ConfigFilePath("pods")
	...
	if input.K8s.DataPersistence == "local" {
		inputPaths = []string{alpineYamlPath}
		k8s = K8s{Action: "apply", Input: inputPaths}
		_, err = client.ExecuteK8sCommand(k8s.Args(kubeConfigPath), true)
		if err != nil {
	...
	err = k8sType("delete", "secrets", "genesisblock")
	if err != nil {
		log.Println("Failed to delete secret genesisblock")
	}

	err = k8sType"delete", "configmaps", "certsparser")
	if err != nil {
...
func k8sType(action, type, name, kubeConfigPath string) error{
	var err error
	args := []string{action, type, name}
	k8s := K8s{Action: "", Input: input}
	err = client.ExecuteK8sCommand(k8s.Args(kubeConfigPath))
	if err != nil {
		return err
	}
	return err
...
func deleteSecrets(numComponents int, componentType, orgName, kubeConfigPath, tls string) error{
	...
	input := []string{"delete", "secrets", componentsList...}
	k8s := K8s{Action: "", Input: input}
	err := client.ExecuteK8sCommand(k8s.Args(kubeConfigPath))
	if err != nil {
		return err
