...
func (d DockerCompose) DockerServicePort(serviceName string, forHealth bool) (string, error) {
	...
	args := []string{"inspect", "-f", `"{{json .NetworkSettings.Ports}}"`, serviceName}
	...
	if err != nil {
		logger.INFO("Failed to unmarshall containerPorts object")
	}
	ports := utils.GetKeysFromMap(containerPorts.Ports)
	port = containerPorts[ports[0]]
	if forHealth {
		port = containerPorts[ports[1]]
	}
	return port, nil
}
