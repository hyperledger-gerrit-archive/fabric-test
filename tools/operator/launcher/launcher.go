...
func checkConsensusType(input networkspec.Config) {
	...
	if ordererType == "solo" {
		if !(len(input.OrdererOrganizations) == 1 && input.OrdererOrganizations[0].NumOrderers == 1) {
			logger.ERROR(nil, "Consensus type solo should have only one orderer organization and one orderer")
		}
	} else if ordererType == "kafka" {
		if len(input.OrdererOrganizations) != 1 {
			logger.ERROR(nil, "Consensus type kafka should have only one orderer organization")
		}
	}
}
