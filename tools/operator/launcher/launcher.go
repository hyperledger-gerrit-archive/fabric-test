...
func validateArguments(networkSpecPath *string, kubeConfigPath *string) {
	...
	if *networkSpecPath == "" {
		logger.ERRORln(nil, "Input file not provided")
...
func doAction(action string, input networkspec.Config, kubeConfigPath string) {
	...
	case "up":
		...
		if err != nil {
			logger.ERRORf(err, "Failed to generate yaml files")
		}
		err = nl.GenerateCryptoCerts(input, kubeConfigPath)
		if err != nil {
			logger.ERRORf(err, "Failed to generate certificates")
		}

		if kubeConfigPath != "" {
			err = nl.CreateMSPConfigMaps(input, kubeConfigPath)
			if err != nil {
				logger.ERRORf(err, "Failed to create config maps")
			}
		}

		err = nl.GenerateGenesisBlock(input, kubeConfigPath)
		if err != nil {
			logger.ERRORf(err, "Failed to create orderer genesis block")
		}

		err = client.GenerateChannelTransaction(input, configFilesPath)
		if err != nil {
			logger.ERRORf(err, "Failed to create channel transactions")
		}

		if kubeConfigPath != "" {
			err = nl.LaunchK8sComponents(kubeConfigPath, input.K8s.DataPersistence)
			if err != nil {
				logger.ERRORf(err, "Failed to launch k8s components")
			}
		} else {
			err = nl.LaunchLocalNetwork()
			if err != nil {
				logger.ERRORf(err, "Failed to launch docker containers")
			}
		}

		err = health.VerifyContainersAreRunning(kubeConfigPath)
		if err != nil {
			logger.ERRORf(err, "Failed to check container status")
		}

		err = health.CheckComponentsHealth("", kubeConfigPath, input)
		if err != nil {
			logger.ERRORf(err, "Failed to check health of fabric components")
		}

		err = connectionprofile.GenerateConnectionProfiles(input, kubeConfigPath)
		if err != nil {
			logger.ERRORf(err, "Failed to create connection profile")
		}
	...
	case "down":
		...
		if err != nil {
			logger.ERRORf(err, "Failed to clean up the network")
		}

	default:
		logger.ERRORf(nil, "Incorrect action", action, "Use up or down for action")
	}
}
...
func checkConsensusType(input networkspec.Config) {
	...
	if ordererType == "solo" {
		if !(len(input.OrdererOrganizations) == 1 && input.OrdererOrganizations[0].NumOrderers == 1) {
			logger.ERRORln(nil, "Consensus type solo should have only one orderer organization and one orderer")
		}
	} else if ordererType == "kafka" {
		if len(input.OrdererOrganizations) != 1 {
			logger.ERRORln(nil, "Consensus type kafka should have only one orderer organization")
		}
	}
}
...
func main() {
	...
	if err != nil {
		logger.ERRORf(err, "In-correct input file path")
	}
