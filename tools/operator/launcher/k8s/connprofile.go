...
func (k K8s) K8sExternalIP(config networkspec.Config, serviceName string) (string, error) {
	...
	if config.K8s.ServiceType == "NodePort" {
		...
		if err != nil {
			logger.INFO("Failed to unmarshall nodePortIP object")
		}
		...
		output, err := client.ExecuteK8sCommand(k.Args(), false)
		if err != nil {
			logger.INFO("Failed to get the external IP for k8s using LoadBalancer")
			return "", err
		}
		err = json.Unmarshal([]byte(output), &loadBalancerIP)
		if err != nil {
			logger.INFO("Failed to unmarshall loadBalancer object")
		}
...
func (k K8s) K8sServicePort(serviceName, serviceType string, forHealth bool) (string, error) {
	...
	if err != nil {
		logger.INFO("Failed to unmarshall k8sPort object")
	}
	portNumber = k8Ports.Spec.Ports[index].NodePort
...
func (k K8s) ordererOrganizations(config networkspec.Config) (map[string]networkspec.Orderer, error) {
	...
	for org := 0; org < len(config.OrdererOrganizations); org++ {
		...
		for i := 0; i < ordererOrg.NumOrderers; i++ {
			...
			portNumber, err = k.K8sServicePort(ordererName, config.K8s.ServiceType, false)
			if err != nil {
				return orderers, err
			}
			nodeIP, err = k.K8sExternalIP(config, ordererName)
			if err != nil {
				return orderers, err
			}
...
func (k K8s) certificateAuthorities(peerOrg networkspec.PeerOrganizations, config networkspec.Config) (map[string]networkspec.CertificateAuthority, error) {
	...
	for i := 0; i < peerOrg.NumCA; i++ {
		...
		portNumber, err = k.K8sServicePort(caName, config.K8s.ServiceType, false)
		if err != nil {
			return CAs, err
		}
		nodeIP, err = k.K8sExternalIP(config, caName)
		if err != nil {
			return CAs, err
		}
...
func (k K8s) peersPerOrganization(peerorg networkspec.PeerOrganizations, config networkspec.Config) (map[string]networkspec.Peer, error) {
	...
	protocol := "grpc"
	peerOrgsLocation := utils.PeerOrgsDir(config.ArtifactsLocation)
	if config.TLS == "true" || config.TLS == "mutual" {
		protocol = "grpcs"
	}
	for i := 0; i < peerorg.NumPeers; i++ {
		peerName := fmt.Sprintf("peer%d-%s", i, peerorg.Name)
		portNumber, err = k.K8sServicePort(peerName, config.K8s.ServiceType, false)
		if err != nil {
			return peers, err
		}
		nodeIP, err = k.K8sExternalIP(config, peerName)
		if err != nil {
			return peers, err
		}
