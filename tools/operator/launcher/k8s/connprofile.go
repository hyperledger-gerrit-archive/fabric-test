...
func (k K8s) K8sExternalIP(input networkspec.Config, serviceName string) (string, error) {
	...
	if input.K8s.ServiceType == "NodePort" {
		...
		k8s.Input = inputArgs
		output, err := client.ExecuteK8sCommand(k8s.Args(k.KubeConfigPath), false)
		if err != nil {
...
func (k K8s) ordererOrganizations(input networkspec.Config) (map[string]networkspec.Orderer, error) {
	...
	for org := 0; org < len(input.OrdererOrganizations); org++ {
		...
		for i := 0; i < ordererOrg.NumOrderers; i++ {
			...
			portNumber, err = K8sServicePort(ordererName, input.K8s.ServiceType, false)
			if err != nil {
				return orderers, err
			}
			NodeIP, err = K8sExternalIP(input, ordererName)
			if err != nil {
				return orderers, err
			}
...
func (k K8s) certificateAuthorities(peerOrg networkspec.PeerOrganizations, input networkspec.Config) (map[string]networkspec.CertificateAuthority, error) {
	...
	for i := 0; i < peerOrg.NumCA; i++ {
		...
		portNumber, err = K8sServicePort(k.KubeConfigPath, caName, input.K8s.ServiceType, false)
		if err != nil {
			return CAs, err
		}
		NodeIP, err = K8sExternalIP(k.KubeConfigPath, input, caName)
		if err != nil {
			return CAs, err
		}
...
func (k K8s) peersPerOrganization(peerorg networkspec.PeerOrganizations, input networkspec.Config) (map[string]networkspec.Peer, error) {
	...
	for i := 0; i < peerorg.NumPeers; i++ {
		...
		portNumber, err = K8sServicePort(k.KubeConfigPath, peerName, input.K8s.ServiceType, false)
		if err != nil {
			return peers, err
		}
		NodeIP, err = K8sExternalIP(k.KubeConfigPath, input, peerName)
		if err != nil {
			return peers, err
		}
...
func (k K8s) GenerateConnectionProfiles(input networkspec.Config) error {
	...
	orderersMap, err := k.ordererOrganizations(input, kubeconfigPath)
	if err != nil {
		return err
	}
	...
	for org := 0; org < len(input.PeerOrganizations); org++ {
		...
		err = generateConnProfilePerOrg(peerorg.Name)
		if err != nil {
