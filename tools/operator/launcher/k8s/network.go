...
func (k K8s) K8sNetwork(action string) error{
	...
	case "up":
		...
		err = network.Generate(k.Config)
		if err != nil{
			logger.CRIT(err)
		}
		err = k.CreateMSPConfigMaps(k.Config, k.KubeConfigPath)
		if err != nil{
		...
		if err != nil{
			logger.CRIT(err, "Failed to verify fabric K8s pods state")
		}
		err = k.CheckK8sComponentsHealth(k.Config, k.KubeConfigPath)
		if err != nil{
			logger.CRIT(err, "Failed to check fabric K8s pods health")
		}
	...
	case "down":
		...
		if err != nil{
			logger.CRIT(err, "Failed to down K8s fabric network")
		}
	default:
