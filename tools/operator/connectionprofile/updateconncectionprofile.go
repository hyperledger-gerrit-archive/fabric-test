...
func (c ConnProfile) updateConnectionProfile(inputArgs ...string) error {
	...
	case "chaincodes":
		channelObject := connProfileObject.Channels[channelName]
		connProfileObject.Channels[channelName] = networkspec.Channel{Orderers: channelObject.Orderers, Peers: channelObject.Orderers, Chaincodes: []string{inputArgs[len(inputArgs)-1]}}
	}
