...
func (c ConnProfile) updateConnectionProfilePerChannel(inputObject interface{}, organizations []inputStructs.Organization, action string) error {
	...
	for _, orgName := range orgNames {
		...
		if configObject.ChannelPrefix != "" {
			err = c.updateConnectionProfilesIfChanPrefix(inputObject, organizations, action, orgName)
			if err != nil {
				return err
			}
		} else {
			err = c.updateConnectionProfilePerOrg(organizations, action, orgName, configObject.ChannelName)
			if err != nil {
				return err
			}
		}
...
func (c ConnProfile) updateConnectionProfilesIfChanPrefix(inputObject interface{}, organizations []inputStructs.Organization, action, orgName string) error {

	var err error
	configObject, _ := inputObject.(*inputStructs.Channel)
	var channelName string
	for i := 0; i < configObject.NumChannels; i++ {
		channelName = fmt.Sprintf("%s%d", configObject.ChannelPrefix, i)
