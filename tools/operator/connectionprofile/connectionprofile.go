...
func (c ConnProfile) Organization(peerorg networkspec.PeerOrganizations, caList []string) networkspec.Organization {
	...
	peerOrgsLocation := utils.PeerOrgsDir(c.Input.ArtifactsLocation)
	...
	organization.Peers = append(organization.Peers, c.getKeysFromMap(c.Peers)...)
	return organization
}
...
func (c ConnProfile) GenerateConnProfilePerOrg(orgName string) error {
	...
	var err error
	path := utils.ConnectionProfilesDir(c.Input.ArtifactsLocation)
	fileName := utils.JoinPath(path, fmt.Sprintf("connection_profile_%s.yaml", orgName))
	channels := make(map[string]networkspec.Channel)
	for i := 0; i < c.Input.NumChannels; i++ {
		var channel networkspec.Channel
		orderersList := c.getKeysFromMap(c.Orderers)
		peersList := c.getKeysFromMap(c.Peers)
	...
	if err != nil {
		logger.INFO("Failed to convert the connection profile struct to bytes")
		return err
	}
	_, err = os.Create(fileName)
	if err != nil {
		logger.INFO("Failed to create ", fileName)
		return err
	}
	yamlBytes = append([]byte("version: 1.0 \nname: My network \ndescription: Connection Profile for Blockchain Network \n"), yamlBytes...)
	err = ioutil.WriteFile(fileName, yamlBytes, 0644)
	if err != nil {
		logger.INFO("Failed to write content to ", fileName)
		return err
	}
...
func (c ConnProfile)getKeysFromMap(newMap interface{}) []string {
