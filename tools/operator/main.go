...
func doAction(action, kubeConfigPath, componentName string, input networkspec.Config) {
	...
	case "createChannelTxn":
		...
		if err != nil {
			logger.ERRORf(err, "Failed to create channel transaction")
		}
	case "migrate":
		err := client.MigrateToRaft(input, kubeConfigPath)
		if err != nil {
			logger.ERRORf(err, "Failed to migrate consensus to raft from", input.Orderer.OrdererType)
		}
	case "healthz":
		err := client.CheckComponentsHealth(componentName, kubeConfigPath, input)
		if err != nil {
			logger.ERRORf(err, "Failed to get the health for", componentName, )
		}
	default:
		logger.ERRORf(nil, "Incorrect mode (%s). Use createChannelTxn or migrate for mode", action)
	}
}
