...
type Deploy struct {
	...
	Language      ChannelOptions `json:"language,omitempty"`
}
...
func (i InstallChainCodeObject) InstallChainCode(config helper.Config) error {
	...
	for index := 0; index < len(config.InstallCC); i++ {
		ccObjects := i.installChainCodeObjects(config, tls)
...
func (i InstallChainCodeObject) installChainCodeObjects(ccObject helper.InstallCC, organizations []helper.Organization, tls string) []InstallChainCodeObject {
	...
	var deploy Deploy
	i = InstallChainCodeObject{TransType: "install", TLS: tls, ChainCodeVer: ccObject.ChainCodeVersion, ConnProfilePath: i.getConnProfilePathForOrg(orgName, organizations)}
