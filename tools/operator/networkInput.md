   ...
   values used by fabric software. Here is a sample network input file:
   ...
   - Description: `tls` is used for server authentication between fabric nodes. Set to `true` to enable tls; set to   `mutual` to use server-client authentication; set to `false` to not enable tls between fabric nodes
   ...
   To scrape fabric metrics, prometheus has to be launched prior to fabric network. Set to `true` to enable scraping of fabric metrics; set to `false` to disable scraping of fabric metrics
   ...
   kafka cluster
      ...
      - Supported Values: Value to launch number of kafka in fabric network. Supports value to be
      3 or higher
      - Example: `num_kafka: 5`
      ...
      - Supported Values: Value to set number of kafka replications. Value should be <= `num_kafka`
   ...
   fabric network. All orderers in all orderer organizations will participate in every channel.
   ...
   using `testorgschannel` as base name. Any fabric sdk client can be used to submit them to create channels
   ...
   will be ignored while launching fabric network locally
      ...
      - Description: `service_type` is used to set type of service to be used for pods in kubernetes.
      Refer to <https://kubernetes.io/docs/concepts/services-networking/service/> for types of services
      ...
      - Description: `resources` section is used to list pod resources for fabric components like orderers, peers, kafka, couchdb and dind (part of the peer pod which runs all chaincode containers of a peer)

         ##### limits

         - Description: `limits` is used to set maximum limits on cpu and memory of a container in pod

            ###### cpu

            - Description: `cpu` is used to set maximum cpu limits of a container in pod
            ...
            - Description: `memory` is used to set maximum memory limits of a container in pod
         ...
         - Description: `requests` is used to set minimum resources required for a container in pod to start

            ###### cpu

            - Description: `cpu` in `requests` is used to set minimum cpu's required for the container in pod to get started
            - Supported Values: Refer to <https://kubernetes.io/docs/tasks/configure-pod-container/assign-cpu-resource/> for setting cpu resources
            - Example: `cpu: "1"`

            ###### memory

            - Description: `memory` in `requests` is used to set minimum memory required for the container in pod to get started
            - Supported Values: Refer to <https://kubernetes.io/docs/tasks/configure-pod-container/assign-memory-resource/> for setting memory resources
            - Example: `memory: 1Gi`

         ##### orderers:

         - Description: `orderer` Section is used to list pod resources for all orderers in fabric network.
         For example:
         ```
         orderers:
            limits:
         ...
         - Description: `peers` Section is used to list pod resources for all peers in fabric network.
         For example:
         ```yaml
         peers:
            limits:
         ...
         - Description: `dind` Section is used to list pod resources for all dind's inside peer pods in fabric network.
         For example:
         ```yaml
         dind:
            limits:
         ...
         - Description: `couchdb` Section is used to list pod resources for couchdb container inside peer pods in fabric network when `db_type` is set as `couchdb`
         For example:
         ```yaml
         couchdb:
            limits:
         ...
         - Description: `kafka` Section is used to list pod resources for all kafka's in fabric network when `orderertype` is set as `kafka`
         For example:
         ```yaml
         kafka:
            limits:
