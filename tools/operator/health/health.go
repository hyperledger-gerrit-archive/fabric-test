...
func checkHealth(componentName, kubeconfigPath string, input networkspec.Config) error {
	...
	logger.INFO("Response status:", string(resp.StatusCode))
	logger.INFO("Response body: %s", healthStatus)
...
func checkK8sContainerState(kubeconfigPath string) error {
    ...
    for i := 0; i < 10; i++ {
        ...
        logger.INFO("Waiting up to 10 minutes for pods to be up and running; minute", string(i))
