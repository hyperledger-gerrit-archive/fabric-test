...
samplecc_go_4M_payload() {
  cd "$PTEDir"/CITest/scripts || exit 1
  echo "-------> Execute Invoke"
  ./gen_cfgInputs.sh -d "$ConnProfile" --nchan 2 --chan0 0 --norg 2 --chanprefix testorgschannel --chantxpath "$Chantxpath" --tls "$1" --payloadmin 4000000 --payloadmax 4000000 -a sample_cc --freq 1000 --nreq 10 --nproc 1 --targetpeers ORGANCHOR -t move > "$LogsDir"/"$2"_samplecc_go_4M_i.log
  sleep 60
