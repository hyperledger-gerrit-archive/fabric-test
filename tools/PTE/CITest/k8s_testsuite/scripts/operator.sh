...
samplecc_go_50MB_TX() {
  cd "$PTEDir"/CITest/scripts || exit 1
  echo "-------> Execute Invoke"
  ./gen_cfgInputs.sh -d "$ConnProfile" --nchan 1 --chan0 0 --norg 1 --chanprefix testorgschannel --chantxpath "$Chantxpath" --tls "$1" --payloadmin 26214400 --payloadmax 26214400 -a sample_cc --freq 10000 --nreq 2 --nproc 1 --targetpeers ORGANCHOR -t move > "$LogsDir"/"$2"_samplecc_go_50MB_i.log
  sleep 60
