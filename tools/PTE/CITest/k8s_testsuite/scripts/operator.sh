...
NLDir="$FabricTestDir"/tools/operator
...
FAB-3833-2i() {
  ...
  echo "-------> Execute Invoke"
  export hfc_logging='{"info":"console"}'
  ./gen_cfgInputs.sh -d "$ConnProfile" --nchan 2 --chan0 0 --norg 1 --chanprefix testorgschannel --chantxpath "$chantxpath" -a samplecc --freq 10 --nreq 10  --nproc 2 --keystart 100 --targetpeers ORGANCHOR -t move > "$PTEDir"/FAB-3833-2i.log
  sleep 60
  cp -r "$PTEDir"/pteReport.txt FAB-3833-2i-pteReport.txt
  node get_pteReport.js FAB-3833-2i-pteReport.txt
  rm -rf "$PTEDir"/pteReport.txt
  echo "-------> Execute Query"
  export hfc_logging='{"info":"console"}'
  ./gen_cfgInputs.sh -d "$ConnProfile" --nchan 2 --chan0 0 --norg 1 --chanprefix testorgschannel --chantxpath "$chantxpath" -a samplecc --freq 10 --nreq 10  --nproc 2 --keystart 100 --targetpeers ORGANCHOR -t query > "$PTEDir"/FAB-3833-2q.log
  sleep 60
  cp -r "$PTEDir"/pteReport.txt FAB-3833-2q-pteReport.txt
  # Convert Test Report into Aggregate summary
  node get_pteReport.js FAB-3833-2q-pteReport.txt
  # remove PTE Report
  rm -rf "$PTEDir"/pteReport.txt
}
...
FAB-4038-2i() {
  cd "$PTEDir"/CITest/scripts
  echo "-------> Execute Invoke"
  ./gen_cfgInputs.sh -d "$ConnProfile" --nchan 2 --norg 1 --chantxpath "$chantxpath" --chanprefix testorgschannel -a samplejs --freq 10 --nreq 10  --nproc 2 --keystart 100 --targetpeers ORGANCHOR -t move > "$PTEDir"/FAB-4038-2i.log
    cp -r "$PTEDir"/pteReport.txt FAB-4038-2i-pteReport.txt
  # Convert Test Report into Aggregate summary
  node get_pteReport.js FAB-4038-2i-pteReport.txt
  # remove PTE Report
  rm -rf "$PTEDir"/pteReport.txt
  sleep 60
  echo "-------> Execute Query"
  ./gen_cfgInputs.sh -d "$ConnProfile" --nchan 2 --norg 1 --chantxpath "$chantxpath" --chanprefix testorgschannel -a samplejs --freq 10 --nreq 10  --nproc 2 --keystart 100 --targetpeers ORGANCHOR -t query > "$PTEDir"/FAB-4038-2q.log
  cp -r "$PTEDir"/pteReport.txt FAB-4038-2q-pteReport.txt
  # Convert Test Report into Aggregate summary
  node get_pteReport.js FAB-4038-2q-pteReport.txt
  # remove PTE Report
  rm -rf "$PTEDir"/pteReport.txt
}
