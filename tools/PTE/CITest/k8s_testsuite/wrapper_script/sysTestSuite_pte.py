...
class System_Tests_Kafka_Couchdb_TLS(unittest.TestCase):
    ...
    def test_5samplecc_go_2chan(self):
        '''
        Description:
        ...
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 40 received 40\" samplecc_go_2chan_i_pteReport.txt | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)

        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 40 received 40 failures 0\" samplecc_go_2chan_q_pteReport.txt | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)


    def test_6samplejs_node_2Chan(self):
        '''
        Description:
        ...
        # check the counts
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 40 received 40\" samplejs_node_2chan_i_pteReport.txt | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)

        # check the counts
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 40 received 40 failures 0\" samplejs_node_2chan_q_pteReport.txt | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)
