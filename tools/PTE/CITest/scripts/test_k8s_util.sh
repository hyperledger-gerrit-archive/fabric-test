...
function printHelp {
   ...
   echo "Usage: "
   echo "This tool is to be used for simple test-related management of HLF networkss in IKS cluster."
   echo "    -s: Stop an orderer. Must be used with -o, -k and -l"
   echo "    -r: Resume all stooped orderers. Must be used with -c, -k and -l"
   ...
   echo " example: "
   echo " (orderer stop) "
   echo "./test_k8s_util.sh -s -o orderer1st-ordererorg -k ~/cello/src/agent/ansible/vars/kubeconfig.regression -l ~/cello/src/agent/ansible/vars"
   echo ""
   echo "(find leader)"
   echo "./test_k8s_util.sh -f -o orderer1st-ordererorg -k ~/cello/src/agent/ansible/vars/kubeconfig.regression -l ~/cello/src/agent/ansible/vars -h testchannel1"
