...
# FUNCTION: usage
#           Displays usage info w command line options; exits.
usage () {
    echo -e "\nUSAGE:\t./get_peerStats.sh [options] [values]"
    echo -e "\tThe output file is <output dir>/<prefix>_<runid>.txt"
    echo
    echo -e "-h, --help\tView this help message."
    echo
    echo -e "-n, --name\tPrefix of logfile name."
    echo -e "\t\t(Default: peer)"
    echo
    ...
    echo -e "\tThe toutput file is myDir/myTest_myTestID.txt"
    echo
    exit
}
