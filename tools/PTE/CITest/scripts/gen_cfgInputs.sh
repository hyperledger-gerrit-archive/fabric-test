...
usage () {
    ...
    echo -e "\t\t(Default: 240000)"

    echo -e "--trequest \trequest ack timeout [unit: ms]"
    echo -e "\t\t(Default: 45000)"

    echo -e "--tgrpc \tgrpc wait timeout [unit: ms]"
    echo -e "\t\t(Default: 3000)"
...
echo "***      REQUEST TIMEOUT: $TREQUEST ms                   "
echo "***      GRPC WAIT TIMEOUT: $TGRPC ms                    "
echo "***                                                      "
...
PreTXProc() {
    ...
    invokeType=${2}
        ...
        sed -i -e "s/_TREQUEST_/$TREQUEST/g" $cfgTX
        sed -i -e "s/_TGRPC_/$TGRPC/g" $cfgTX
        rm -f $cfgTX"-e"
}
...
while [[ $# -gt 0 ]]; do
    ...
    case $arg in
      ...
      --trequest)
          shift
          TREQUEST=$1
          echo -e "\t- Specify request timeout: $TREQUEST\n"
          shift
          ;;
      ...
      --tgrpc)
          shift
          TGRPC=$1
          echo -e "\t- Specify GRPC timeout: $TGRPC\n"
          shift
          ;;
