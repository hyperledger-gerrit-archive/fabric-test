...
usage () {
    ...
    echo -e "--ordmethod\torderer selection method [UserDefined|RoundRobin]"
...
PreTXProc() {
    ...
    ordmethod=${12}
        ...
        sed -i -e "s/_ORDMETHOD_/$ordmethod/g" $cfgTX
        rm -f $cfgTX"-e"
}
    ...
    for chaincode in "${Chaincode[@]}"; do
        ...
        for scfile in "${NWName[@]}"; do
            ...
            for chan in "${CHANNEL[@]}"; do
                ...
                PreTXProc $pteTXopt $INVOKETYPE $NPROC $FREQ $NREQ $RUNDUR $TXMODE $TARGETPEERS $CHKPEERS $CHKTX $CHKTXNUM $ORDMETHOD
...
while [[ $# -gt 0 ]]; do
    ...
    case $arg in
      ...
      --ordmethod)
          shift
          ORDMETHOD=$1
          echo -e "\t- Specify ordererOpt method: $ORDMETHOD\n"
          shift
          ;;
