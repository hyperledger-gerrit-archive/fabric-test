...
if [ $TCase == "robust-i-TLS" ]; then
    # robustness test
    echo "*************** [$0] executing: ***************"
    echo "    ./pte_remote.sh CITest/$TCase/samplecc/PTEMgr-$TCase.txt >& CITest/$TCase/samplecc/$TCase.log"
    sleep 20s
    ./pte_remote.sh CITest/$TCase/samplecc/PTEMgr-$TCase.txt $TSTART >& $CIDir/$TCase/samplecc/$TCase.log &
    ...
    for cc in $ccDir; do
        ...
        for pte in $ptemgr; do
            echo "*************** [$0] executing: ***************"
            echo "    ./pte_remote.sh CITest/$TCase/$cc/$pte > CITest/$TCase/$cc/$pte.log"
            sleep 20s
            ./pte_remote.sh CITest/$TCase/$cc/$pte $TStart > CITest/$TCase/$cc/$pte.log
