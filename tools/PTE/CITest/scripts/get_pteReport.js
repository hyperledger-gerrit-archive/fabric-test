...
// output results
    ...
    if ( transType == 'INVOKE' ) {
        ...
        buff = '        '+hdr+' Overall proposals '+peerLatency[0]+' min '+peerLatency[2]+' ms max '+peerLatency[3]+'ms avg '+avg.toFixed(2)+' ms\n';
        fs.appendFileSync(pteReport, buff);

        avg=ordererLatency[1]/ordererLatency[0];
        buff = '        '+hdr+' Overall transactions '+ordererLatency[0]+' min '+ordererLatency[2]+' ms max '+ordererLatency[3]+' ms avg '+avg.toFixed(2)+' ms\n';
        fs.appendFileSync(pteReport, buff);
        avg=eventLatency[1]/eventLatency[0];
        buff = '        '+hdr+' Overall events '+eventLatency[0]+' min '+eventLatency[2]+' ms max '+eventLatency[3]+' ms avg '+avg.toFixed(2)+' ms\n';
        fs.appendFileSync(pteReport, buff);
