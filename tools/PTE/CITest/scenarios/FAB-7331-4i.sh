#!/bin/bash

#
# Copyright IBM Corp. All Rights Reserved.
#
# SPDX-License-Identifier: Apache-2.0
#

########## CI test ##########

CWD=$PWD
PREFIX="result"   # result log prefix

#### Lauch network and synch-up ledger
cd $GOPATH/src/github.com/hyperledger/fabric-test/fabric-sdk-node/test/PTE/CITest/scripts
./test_driver.sh -n -m FAB-7331-4i -p -c samplejs -t FAB-7204-4q
#### remove PTE log from synch-up ledger run
rm -f ../Logs/FAB-7204-4q*.log
#### execute testcase FAB-7204-4i: 2 threads invokes, golevelDB
./test_driver.sh -t FAB-7331-4i
#### gather TPS from docker containers
./get_peerStats.sh -r FAB-7331-2i -p peer0.org1.example.com peer0.org2.example.com -c testorgschannel1 -n $PREFIX -o $CWD -v
