...
// validate Connection Profile property
function validateConnProfileProp(cpf, prop) {

    if ( cpf === null ) {
       logger.error('[validateConnProfileProp] the connection profile is invalid: null');
       return 0;
    }

    if ( ! cpf.hasOwnProperty(prop) ) {
       logger.error('[validateConnProfileProp] prop (%s) is not found in connectino profile', prop);
       return 0;
    }

    if ( true === isEmpty(cpf[prop]) ) {
       logger.error('[validateConnProfileProp] prop (%s) is empty in connectino profile', prop);
       return 0;
    }
...
module.exports.validateConnProfilePropSubmitter=function(cpf, key) {
    return validateConnProfileProp(cpf, key);
}
...
function findOrgCA(inPtr, org) {
    ...
    if ( 0 === validateConnProfileProp(inPtr, 'certificateAuthorities') ) {
...
function getMember(username, password, client, nid, userOrg, cpf) {
    ...
    if ( 0 === validateConnProfileProp(cpf, 'certificateAuthorities') ) {
...
function getOrdererAdmin(client, userOrg, cpf) {
        ...
        var cpOrgs = cpf['organizations'];
        if ( validateConnProfileProp(cpf, 'orderers') === 0 ) {
...
function getTLSCert(key, subkey, cpf) {
    ...
    if ( validateConnProfileProp(cpf, 'peers') === 0 ) {
...
module.exports.tlsEnroll = async function(client, orgName, cpf) {
  try {
    var cpOrgs = cpf['organizations'];
    if ( validateConnProfileProp(cpf, 'certificateAuthorities') === 0 ) {
