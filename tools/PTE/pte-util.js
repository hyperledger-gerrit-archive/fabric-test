...
function validateOrgConnProfile(cpf, org) {
    if ( cpf == null ) {
       logger.error('[Nid:chan:org:id=%d:%s:%s:%d pte-execRequest] no connection profile is found for org (%s)', Nid, channel.getName(), org, pid, org);
       process.exit(1);
    }
    return;
}
module.exports.validateOrgConnProfileSubmitter=function(cpf, org) {
    return validateOrgConnProfile(cpf, org);
}
...
function findOrgConnProfile(fileList, orgname) {
    ...
    for (i=0; i<fileList.length; i++) {
        ...
        if ( temp['organizations'] && temp['organizations'][orgname] &&
             temp['peers'] && temp['orderers'] && temp['certificateAuthorities']) {
