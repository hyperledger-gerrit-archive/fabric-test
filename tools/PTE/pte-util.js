...
function setFilePtr(inFile, keyreq) {
    var temp;
    var file_ext = path.extname(inFile);
    logger.info('[setFilePtr] input File: ', inFile);
    ...
    if ( temp[keyreq] ) {
        logger.info('[setFilePtr] set pointer to %s[%s] ', inFile, keyreq);
        return temp[keyreq];
    } else {
        logger.info('[setFilePtr] set pointer to %s', inFile);
        return temp;
    }

}

module.exports.setFilePtrSubmitter=function(inFile, keyreq) {
    return setFilePtr(inFile, keyreq);
}
...
function getMember(username, password, client, nid, userOrg, svcFile) {
        ORGS = setFilePtr(svcFile, 'test-network');
...
module.exports.tlsEnroll = async function(client, orgName, svcFile) {
    var orgs = setFilePtr(svcFile, 'test-network');
