...
function getPropFromConnProfiles(fileList, prop) {
    var propList = {};

    logger.info('[getPropFromConnProfiles] input prop (%s), File list: ', prop, fileList);
    for (i=0; i<fileList.length; i++) {
        var cpf = fileList[i];
        var temp = readConfigFile(cpf, 'test-network');
        if ( temp[prop] && ( ! isEmpty(temp[prop]) ) ) {
            let cnt = Object.getOwnPropertyNames(temp[prop]).length;
            for (var key in temp[prop]) {
                let found = 0;
                for (var key1 in propList) {
                    if (temp[prop][key].url === propList[key1].url) {
                        found = 1;
                        break;
                    }
                }
                if ( found === 0 ){
                    propList[key]=temp[prop][key];
                }
            }
        }
    }
    ...
    if ( Object.keys(propList).length == 0 ) {
        logger.warn('[getPropFromConnProfiles] no %s found in all connection profiles', prop);
    }

    logger.info('[getPropFromConnProfiles] %s List: %j', prop, propList);
    return propList;

}
module.exports.getPropFromConnProfilesSubmitter=function(fileList, prop) {
    return getPropFromConnProfiles(fileList, prop);
}
...
// find Connection Profile contain a subkey under section key
function findKeyConnProfile(fileList, key, subkey) {
    logger.info('[findKeyConnProfile] key(%s), subkey(%s): ', key, subkey, fileList);
    var temp = null;
    for (i=0; i<fileList.length; i++) {
        var cpf = fileList[i];
        var temp = readConfigFile(cpf, 'test-network');
        if ( temp[key] && temp[key][subkey] ) {
            logger.info('[findKeyConnProfile] key(%s) subkey(%s) found File: ', key, subkey, cpf);
            break;
        }
    }
    return temp;
...
module.exports.findKeyConnProfileSubmitter=function(fileList, orgname) {
    return findKeyConnProfile(fileList, orgname);
}
...
function getOrdererAdmin(client, userOrg, cpf, cpPath) {
        ...
        orderersCPFList = getPropFromConnProfiles(cpList, 'orderers');
