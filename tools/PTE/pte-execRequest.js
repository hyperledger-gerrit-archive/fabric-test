                ...
                if ( (typeof(filtered_block.number) != 'undefined') && (filtered_block.number > 0) ) {
                    if (typeof(filtered_block.filtered_transactions) != 'undefined') {
                      for (i=0; i<filtered_block.filtered_transactions.length; i++) {
                        ...
                        logger.error('[Nid:chan:org:id=%d:%s:%s:%d eventRegisterFilteredBlock] pte-exec: Failure - received filtered_block.number:%d but filtered_transactions is undefined', Nid, channelName, org, pid, filtered_block);
                        process.exit(1);
                    }
