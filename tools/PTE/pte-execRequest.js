...
if ( transMode == 'DISCOVERY' && TLS != testUtil.TLSCLIENTAUTH ) {
    logger.error('[Nid:chan:org:id=%d:%s:%s:%d pte-execRequest] invalid configuration: transMode (%s) requires mutual TLS (clientauth)', Nid, channelName, org, pid, txCfgPtr.transMode);
    process.exit();
}
