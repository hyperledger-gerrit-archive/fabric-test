...
if ( 0 === testUtil.validateConnProfilePropSubmitter(cpf, 'orderers') ) {
...
if ( 0 === testUtil.validateConnProfilePropSubmitter(cpf, 'peers') ) {
    logger.info('[Nid:chan:org:id=%d:%s:%s:%d pte-execRequest] no orderer found in the connection profile', Nid, channel.getName(), org, pid);
    process.exit(1);
}
...
function assignThreadAllPeers(channel, client, org) {
    ...
    for (var i=0; i<channelOrgName.length; i++ ) {
        ...
        if ( 0 === validateConnProfileProp(cpf, 'peers') ) {
