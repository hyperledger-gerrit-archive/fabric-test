...
testUtil.validateOrgConnProfileSubmitter(cpf, org);
...
function assignThreadAllPeers(channel, client, org) {
    ...
    for (var i=0; i<channelOrgName.length; i++ ) {
        ...
        if ( cpfTmp === null ) {
            logger.info('[Nid:chan:org:id=%d:%s:%s:%d assignThreadPeerList] cannot find connection profile for org (%s)', Nid, channel.getName(), org, pid, orgtmp);
            continue;
        }
