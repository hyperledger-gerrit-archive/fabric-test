...
if ( transMode == 'DISCOVERY' && TLS != testUtil.TLSCLIENTAUTH ) {
    logger.error('[Nid:chan:org:id=%d:%s:%s:%d pte-execRequest] invalid configuration: transMode (%s) requires TLS (clientauth)', Nid, channelName, org, pid, txCfgPtr.transMode);
    process.exit();
}
    ...
    return promise.then((store) => {
        ...
        return testUtil.getSubmitter(username, secret, client, true, Nid, org, svcFile);
	            ...
	            logger.info('[Nid:chan:org:id=%d:%s:%s:%d execTransMode] execTransMode: tCurr= %d, tStart= %d, time to wait=%d', Nid, channelName, org, pid, tCurr, tStart, tSynchUp);
                    ...
                    logger.info('[Nid:chan:org:id=%d:%s:%s:%d execTransMode] execTransMode: serviceDiscovery=%s', Nid, channelName, org, pid, serviceDiscovery);
                    channel.initialize({
    ...
    if ( trType.toUpperCase() == 'MOVE' ) {
        ...
        if ( nRequest > 0 ) {
           if ( (n_sd % (nRequest/10)) == 0 ) {
              logger.info(util.format("[Nid:chan:org:id=%d:%s:%s:%d isExecDone] invokes(%s) sent: number=%d, evtTimeoutCnt=%d, elapsed time= %d",
                                         Nid, channelName, org, pid, trType, n_sd, evtTimeoutCnt, tCurr-tLocal));
           }
...
function invoke_discovery() {
    n_sd++;
    //tCurr = new Date().getTime();
    //logger.info('[Nid:chan:org:id=%d:%s:%s:%d execModeDiscovery] n_sd %d, tCurr %d', Nid, channelName, org, pid, n_sd, tCurr);
    ...
    .then((success) => {
        isExecDone(transMode);
        if ( IDone != 1 ) {
            invoke_discovery();
        } else {
            tCurr = new Date().getTime();
            logger.info('[Nid:chan:org:id=%d:%s:%s:%d invoke_discovery] pte-exec:completed sent %d transactions %s(%s) in %d ms, timestamp: start %d end %d,Throughput=%d TPS', Nid, channelName, org, pid, n_sd, transType, transMode, tCurr-tLocal, tLocal, tCurr,(n_sd/(tCurr-tLocal)*1000).toFixed(2));
        }
    },
    function(err) {
