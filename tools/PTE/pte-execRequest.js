            ...
            txidList[tx_id.getTransactionID().toString()] = new Date().getTime();
                ...
                .then((results) => {
                    ...
                    if ( IDone != 1 ) {
                        ...
                        tCurr = new Date().getTime();
                        var remain = Object.keys(txidList).length;
                        logger.info('[Nid:chan:org:id=%d:%s:%s:%d invoke_move_burst_evtBlock] completed %d, evtTimoutCnt %d, unceived events %d, %s(%s) in %d ms, timestamp: start %d end %d', Nid, channelName, org, pid, inv_m, evtTimeoutCnt, remain, transType, invokeType, tCurr-tLocal, tLocal, tCurr);
                        if ( remain > 0 ) {
    ...
    .then((results) => {
            ...
            .then((results) => {
                ...
                if ( IDone != 1 ) {
                    ...
                    tCurr = new Date().getTime();
                    var remain = Object.keys(txidList).length;
                    logger.info('[Nid:chan:org:id=%d:%s:%s:%d invoke_move_burst] completed %d, evtTimoutCnt %d, unceived events %d, %s(%s) in %d ms, timestamp: start %d end %d', Nid, channelName, org, pid, inv_m, evtTimeoutCnt, remain, transType, invokeType, tCurr-tLocal, tLocal, tCurr);
                    if ( remain > 0 ) {
                        console.log('[Nid:chan:org:id=%d:%s:%s:%d invoke_move_const] unreceived events(%d), txidList', Nid, channelName, org, pid, remain, txidList);
                    }
                    return;
