...
4. User can opt to include `admin_cert` in the json by setting `admin_cert` within each **org** section.  If the `admin_cert` is set, then the files specified in `adminPath` is ignored.  This can simplify the setting of adminPath for a network with large number of org.
