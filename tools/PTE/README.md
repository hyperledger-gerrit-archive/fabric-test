...
1. Create Connection profile(s) for your Fabric network:
    - Create your own version of connection profile, see the example in **PTE/ConnProfiles/config.yaml** and change the address to your own Fabric addresses and credentials. The connection profile can be in either yaml or json format. Add a block for each channel, organization, orderer, peer, and certificateAuthorities, ensuring correctness.
...
You can use `PTE/ConnProfiles/config.yaml` as an example to create your own connection profile. See [Connection Profile](#connection-profile) for detail of the connection profile.
...
* **invoke** invoke transaction contents
    * **query**: query content
    * **move**: move content
* **ConnProfilePath**: path to the connection profiles, all connection profiles listed in this directory will be used by PTE. supported file type: json and yaml, see PTE/ConnProfiles/config.yaml for example
...
## Connection Profile
The connection profile contains the information of the network and are stored in the `PTE/ConnProfiles` directory. The following is a sample of the connection profile yaml file:
