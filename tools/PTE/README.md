...
- transactions: to deliver transactions to the targeted peers with specified transaction mode and type
...
- blockchain information: to query blockchain height and number of transactions
- results: to provide test results
...
3. Checkout appropriate docker images:
    - fabric

        There are two ways to obtain the docker images:
...
6. Create Service Credentials file(s) for your Fabric network:
...
* **run time duration**: Each process executes the same transaction concurrently for the specified time duration specified by runDur in the user input file, note that nRequest is set to 0.  If both nRequest and runDur are set to 0, the process will execute the transaction **forever**.
...
* **runDur**: run duration in seconds to be executed if nRequest is 0. PTE will execute transactions forever if both nRequest and runDur are set to 0.
