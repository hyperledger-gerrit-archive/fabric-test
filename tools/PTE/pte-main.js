                    ...
                    if ( procDone === nProcPerOrg*channelOrgName.length ) {
                        ...
                        for (summaryIndex in testSummaryArray ) {
                            ...
                            if (rawText.indexOf("execTransMode")>-1) {
                                logger.info("ERROR occured:" +rawText);
                                continue;
                            };
                            ...
                            if (rawText.indexOf("invoke_query_")>-1) {
                                if(rawText.indexOf("pte-exec:completed:error") >-1)
                                {
                                    logger.info("ERROR occured:" +rawText);
                                    continue;
                                }

                                var queryTransNum= parseInt(rawText.substring(rawText.indexOf("pte-exec:completed")+18,rawText.indexOf("transcation",rawText.indexOf("pte-exec:completed")+18)).trim());
                        ...
                        logger.info("Test Summary: Totoal %d Threads run completed",procDone);
                        if (totalInvokeTrans>0) {
                            logger.info("Test Summary:Total INVOKE transaction=%d, timeout transcation=%d, the Min duration is %d ms,the Max duration is %d ms,Avg duration=%d ms, total throughput=%d TPS", totalInvokeTrans, totalInvokeTimeout, minInvokeDuration, maxInvokeDuration,totalInvokeTime/procDone, totalInvokeTps.toFixed(2));
                        }
