...
function joinOneChannel(channel, client, org) {
    ...
    joinChannel(channel, client, org)
        .then(() => {
            ...
            throw new Error(util.format('[joinOneChannel] Failed to join peers in organization "%s" to the channel', ORGS[org].name))
        })
        .catch(function(err) {
                ...
                .then((store) => {
                    ...
                    testUtil.getSubmitter(username, secret, client, true, Nid, org, svcFile)
                        .then(
                            ...
                            function(err) {
                                evtDisconnect();
                                throw new Error('[Nid=%d] Failed to wait due to error: ', Nid, err.stack ? err.stack : err)
                            }
                        );
                }).catch((err) => {
                ...
                .then((store) => {
                    ...
                    testUtil.getSubmitter(username, secret, client, true, Nid, org, svcFile)
                        .then(
                            ...
                            function(err) {
                                evtDisconnect();
                                throw new Error('[performance_main:Nid=%d] Failed to wait due to error: ', Nid, err.stack ? err.stack : err)
                            }
                        );
                }).catch((err)=>{
                    logger.error(err)
