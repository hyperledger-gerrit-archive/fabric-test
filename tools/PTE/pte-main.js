...
async function performance_main() {
    ...
    for (var i=0; i<channelOrgName.length; i++ ) {
        ...
        if ( transType == 'INSTALL' ) {
            ...
            var channelAction = channelOpt.action.toUpperCase();
            if (channelAction == 'CREATE') {
                // create channel once
                if (channelCreated == 0) {
                    createOrUpdateOneChannel(client, channelOrgName, channelAction);
                    channelCreated = 1;
                } else {
                }
            } else if (channelAction == 'UPDATE') {
                // beware: the specified channel must already exist for the update to succeed; otherwise you would get an error.
                createOrUpdateOneChannel(client, channelOrgName, channelAction);
