...
logger.debug('[Nid=%d pte-main] connection profile path: ', Nid, cpPath);
	...
	try {
		...
		hfc.newDefaultKeyValueStore({
			...
			.then((results) => {
				...
				while(retry<10){
					try{
						return channel.getGenesisBlock(sysCh_request)
					} catch(err){
						if ( retry < 10 ) {
							retry++
						} else {
...
async function joinChannel(channel, client, org) {
	try {
		...
		return hfc.newDefaultKeyValueStore({
			...
			let ch_tx_id = client.newTransactionID();
			var ch_request = {
				txId :  ch_tx_id
			};
			...
			// async.retry({times: 60, interval: 1000}, channel.getGenesisBlock.bind(request), function(err, result) {
			// 	if (err) {
			// 		logger.warn('[systemChannel=%s] Network is not ready yet, please try later or something wrong with network', channelName);
			// 		//process.exit(1);
			// 	} else {
			// 		console.log("=============== success block ================")
			// 		logger.info('[systemChannel=%s] Successfully got the system channel genesis block', channelName, org);
			// 		return result
			// 	}
			// });
			let retry = 0;
			while(retry<10){
				try{
					return channel.getGenesisBlock(ch_request)
				} catch(err){
					if ( retry < 10 ) {
						retry++
					} else {
		...
		// }).then((nothing) => {
		// 	return channel.getGenesisBlock.bind(ch_request)
		}).then((block) => {
