...
//copied createChannel - to edit as updateChannel...
//async function createOneChannel(client ,channelOrgName) {
async function updateChannel(channel, client, org) {
//  After creating channels and joining peers, we can send channel config updates
//  to an orderer, which will deliver a block with the config transaction to all
//  peers that have joined the channel.
//
//  One example is sending anchor peer config updates in each org of each channel.
//  When using networkLauncher and cryptogen, we might see the following:
...
//
//      Notes from reviewing byfn scripts:
//      1.  Do we need to sign it here, or is it already signed?
//            peer channel signconfigtx -f $anchorTx
//          Maybe not, because networkLauncher created the anchorTx with the -asOrg option,
//          and the tools default settin is to require only one admin to sign an update.
//          But then again, maybe we should have the org sign it regardless, anyways...
//
//      2.  With no tls:
//            peer channel update -o orderer.example.com:7050 -c $channel -f $anchorTx
//          Or, with TLS:
//            peer channel update -o orderer.example.com:7050 -c $channel -f $anchorTx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA
//          Or, with Mutual TLS:
//            ?

    // for each org in the channelOpt orgName list, this function will be called.
    // Only one org must be provided, and it should be a member of the channel.
    // Find an orderer, and find the first peer in the provided org.
    // (The peer must be already in the channel!)
    // Then send each of the transactions in the channelUpdateTX list to
    // an orderer, similar to a "peer channel update". And remember to
    // register for an event from the peer, to be sure it gets delivered.
    ...
    // If user defines goPath, then they must also specify path relative to gopath in the input json file.
    // In that case, the PTE must prepend GOPATH/src here.  Otherwise PTE uses the updateTX exactly as
    // specified (user should specify an absolute path) in the input json file.
    var channelUpdateTXs = [];
    for (i=0; i<channelOpt.channelUpdateTX.length; i++) {
        channelUpdateTXs[i] = channelOpt.channelUpdateTX[i];
        if ( goPath !== '' ) {
            channelUpdateTXs[i] = path.join(goPath, 'src', channelOpt.channelUpdateTX[i]);
        }
    }
    logger.info('channelUpdateTX.length: %d, channelUpdateTXs: %s', channelUpdateTX.length, channelUpdateTXs);

// we have the list of updates, and an org, and a channel.
// let's find an orderer, and a first peer in the org (hopefully in the channel),
// and send each of the update Transactions to the orderer.
// Assume the TXs are already signed.
// ---> look at e2e or other examples, to be sure the "peer channel update -asOrg" does indeed sign it.
// If not, then alternatively, we could enter this procedure with same arglist as createOneChannel,
// instead of what we are doing, and look at both the orgName list and channelUpdateTX list
// more closely... and have the corresponding org sign the corresponding updateTX before we send it.
// Or... can we extract the signatures like we extractChannelConfig ?

  for (i=0; i<channelUpdateTXs.length; i++) {
    updateTX = channelUpdateTXs[i];

    envelope_bytes = fs.readFileSync(updateTX);
    config = client.extractChannelConfig(envelope_bytes);
    logger.info('[updateChannel] Successfully extracted the config update from the configtx envelope: ', updateTX);

// we also need to get the signatures here... I deleted a bunch of code from the createOneChannel,
// but we may need that again, if we cannot get the signs out of the ChannelConfig envelope.

    // get client key
    if ( TLS == testUtil.TLSCLIENTAUTH ) {
        await testUtil.tlsEnroll(client, org, svcFile);
        logger.info('[updateChannel] get user private key: org= %s', org);
    }

//  we need to get a newOrderer
//  chainAddOrderer(channel, client, org);
//  or else do this, which is what createOneChannel() did:
    clientNewOrderer(client, org);


// Somewhere in here we need to find a peer in the given org,
// and register for an event for our channel update transaction...

 /*
// Z notes... to add a peer - do we need somewthing like this???
    for (let key in ORGS[org]) {
        if (ORGS[org].hasOwnProperty(key)) {
            if (key.includes('peer')) {
// how can we check if that peer has joined our channel? and the org itself is in the channel?
                if (TLS > testUtil.TLSDISABLED) {
                    data = testUtil.getTLSCert(org, key);
                    if ( data !== null ) {
                        peerTmp = client.newPeer(
                        ...
                        targets.push(peerTmp);
                        //channel.addPeer(peerTmp);
                        //Instead, must call (and write) something like:  channel.configUpdate(peerTmp);
                    }
                } else {
                    peerTmp = client.newPeer( ORGS[org][key].requests);
                    targets.push(peerTmp);
...
// do we just need one promise?
        var submitPromises= [];
        submitter = new Promise(function (resolve,reject) {
                username=ORGS[org].username;
                secret=ORGS[org].secret;
                orgName = ORGS[org].name;
                logger.info('[updateChannel] org= %s, org name= %s', org, orgName);
                client._userContext = null;
                resolve(testUtil.getSubmitter(username, secret, client, true, Nid, org, svcFile));
        });
        submitPromises.push(submitter);
        return Promise.all(submitPromises);
    })
    .then((results) => {
        // build up the config-update request
        let tx_id = client.newTransactionID();
        let nonce = tx_id.getNonce();
        var request = {
            config: config,
        ...
        logger.info('[updateChannel] Successfully updated the channel (%s) with TX id (%s)', channelName, tx_id);
        evtDisconnect();
        process.exit();
    }, (err) => {
        logger.error('Failed to update the channel (%s) ', channelName);
        logger.error('Failed to update the channel:: %j '+ err.stack ? err.stack : err);
        evtDisconnect();
        process.exit();
    })
    .then((nothing) => {
