                ...
                workerProcess.on('exit', function (code) {
                    ...
                    if ( procDone === nProcPerOrg*channelOrgName.length ) {
                        ...
                        if (totalInvokeTrans>0) {
                            logger.info("Test Summary (%s):Total INVOKE transaction=%d, timeout transaction=%d, the Min duration is %d ms,the Max duration is %d ms,Avg duration=%d ms, total throughput=%d TPS", chaincode_id, totalInvokeTrans, totalInvokeEventTimeout, minInvokeDuration, maxInvokeDuration,totalInvokeTime/procDone, totalInvokeTps.toFixed(2));

                            var dur=etmp-stmp;
                            var iTPS=1000*totalInvokeTrans/dur;
                            logger.info("Aggregate Test Summary (%s):Total INVOKE transaction %d, timeout transaction %d, start %d end %d duration is %d ms, TPS %d", chaincode_id, totalInvokeTrans, totalInvokeEventTimeout, stmp, etmp, dur, iTPS.toFixed(2));
