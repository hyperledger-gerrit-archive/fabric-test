    ...
    try {
        ...
        hfc.newDefaultKeyValueStore({
            ...
            .then((results) => {
                ...
                while (retry < 10) {
                    try {
                        let block = await sysChannel.getGenesisBlock(sysCh_request)
                        break
                    } catch (err) {
                        if (retry < 10) {
                            await sleep(1000);
                            ...
                            logger.error('[createOrUpdateOneChannel] Unable to fetch genesis block for system channel, Please try later');
                            process.exit(1);
                        }
                    }
                }
