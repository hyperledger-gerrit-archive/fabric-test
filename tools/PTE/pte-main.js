                ...
                workerProcess.on('exit', function (code) {
                    ...
                    if ( procDone === nProcPerOrg*channelOrgName.length ) {
                        ...
                        if (totalInvokeTrans>0) {
                            ...
                            // transaction output
                            var buff = loggerMsg+" Test Summary:\n";
                            fs.appendFileSync(rptFile, buff);
                            buff = "("+chaincode_id+"): INVOKE transaction stats\n";
                            fs.appendFileSync(rptFile, buff);
                            buff = "("+chaincode_id+"):\tTotal transactions "+totalInvokeTrans + "  timeout transactions "+totalInvokeTimeout+"\n";
                            fs.appendFileSync(rptFile, buff);

                            buff = "("+chaincode_id+"):\tstart "+stmp+"  end "+etmp+"  duration "+dur+" ms \n";
                            fs.appendFileSync(rptFile, buff);
                            buff = "("+chaincode_id+"):\tTPS "+iTPS.toFixed(2)+ "\n";
                            fs.appendFileSync(rptFile, buff);

                            // peer latency output (endorsement)
                            buff = "("+chaincode_id+"): peer latency stats (endorsement)\n";
                            fs.appendFileSync(rptFile, buff);
                            buff = "("+chaincode_id+"):\ttotal transactions: "+ latency_peer[0] +"  total time: "+ latency_peer[1] +" ms \n";
                            fs.appendFileSync(rptFile, buff);
                            if ( latency_peer[0] > 0 ) {
                                buff = "("+chaincode_id+"):\tmin: "+ latency_peer[2] +" ms  max: "+ latency_peer[3] +" ms  avg: "+ latency_peer[1]/latency_peer[0].toFixed(2) +" ms \n";
                                fs.appendFileSync(rptFile, buff);
                            }

                            // orderer latency output (transaction ack)
                            buff = "("+chaincode_id+"): orderer latency stats (transaction ack)\n";
                            fs.appendFileSync(rptFile, buff);
                            buff = "("+chaincode_id+"):\ttotal transactions: "+ latency_orderer[0] +"  total time: "+ latency_orderer[1] +" ms \n";
                            fs.appendFileSync(rptFile, buff);
                            if ( latency_orderer[0] > 0 ) {
                                buff = "("+chaincode_id+"):\tmin: "+ latency_orderer[2] +" ms  max: "+ latency_orderer[3] +" ms  avg: "+ latency_orderer[1]/latency_orderer[0].toFixed(2) +" ms \n";
                                fs.appendFileSync(rptFile, buff);
                            }

                            // event latency output (end-to-end)
                            buff = "("+chaincode_id+"): event latency stats (end-to-end)\n";
                            fs.appendFileSync(rptFile, buff);
                            buff = "("+chaincode_id+"):\ttotal transactions: "+ latency_event[0] +"  total time: "+ latency_event[1] +" ms \n";
                            fs.appendFileSync(rptFile, buff);
                            if ( latency_event[0] > 0 ) {
                                buff = "("+chaincode_id+"):\tmin: "+ latency_event[2] +" ms  max: "+ latency_event[3] +" ms  avg: "+ latency_event[1]/latency_event[0].toFixed(2) +" ms \n\n";
                                fs.appendFileSync(rptFile, buff);
                            }
                        }
                        if (totalQueryTrans>0) {
                            ...
                            var qTPS=1000*totalQueryTrans/(etmp-stmp);
                            logger.info("Aggregate Test Summary (%s):Total QUERY transaction %d, start %d end %d duration is %d ms, TPS %d", chaincode_id, totalQueryTrans, stmp, etmp, etmp-stmp, qTPS.toFixed(2));

                            buff = 'Test Summary ('+chaincode_id+'):Total QUERY transaction '+totalQueryTrans+', start '+stmp+'+ end '+etmp+'+ duration is '+etmp-stmp+' ms, TPS '+qTPS.toFixed(2);
                            fs.appendFileSync(rptFile, buff);
                        }
                        if (totalMixedTPS) {
                            logger.info("Test Summary (%s):Total mixed transaction=%d, the Min duration is %d ms,the Max duration is %d ms,Avg duration=%d ms, total throughput=%d TPS", chaincode_id, totalMixedInvoke+totalMixedQuery , minMixedDuration, maxMixedDuration, (totalMixedTime)/(procDone),(totalMixedTPS).toFixed(2));
                            var mTPS=1000*(totalMixedInvoke+totalMixedQuery)/(etmp-stmp);
