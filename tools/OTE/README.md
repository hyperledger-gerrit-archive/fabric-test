...
```bash
  cd $GOPATH/src/github.com/hyperledger/fabric/bddtests/regression/go/ote
  go build
  ./ote
...
```bash
  ...
  go test -run ORD77
  go test -run ORD7[79]
  go test -run batchSz -timeout 30m
  go test -timeout 90m
...
#### Example command to execute all the acceptance tests identified for running for continuous improvement process, and convert to xml:
```
  go test -v -timeout 30m -run _CI | go-junit-report > ote_report.xml
  CONFIGTX_ORDERER_BATCHSIZE_MAXMESSAGECOUNT=500 go test -v -timeout 120m -run batchSz_CI | go-junit-report > ote_report_batchSz.xml
```
