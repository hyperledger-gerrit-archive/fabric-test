...
func startConsumer(serverAddr string, chanID string, ordererIndex int, channelIndex int, txRecvCntrP *int64, blockRecvCntrP *int64, consumerConnP **grpc.ClientConn, seek int, quiet bool, tlsEnabled bool, orgMSPID string) {
        ...
        if ordererIndex == 0 {
                ...
                if err != nil { // Handle errors reading the config file
                        fmt.Printf("\nFailed to initialize local MSP:%s, %d, %d, %s\n", err, ordererIndex, channelIndex, chanID)
                        os.Exit(0)
                }
        }
        if seek < -2 {
...
func reportTotals(testname string, numTxToSendTotal int64, countToSend [][]int64, txSent [][]int64, totalNumTxSent int64, txSentFailures [][]int64, totalNumTxSentFailures int64, batchSize int64, txRecv [][]int64, totalTxRecv []int64, totalTxRecvMismatch bool, blockRecv [][]int64, totalBlockRecv []int64, totalBlockRecvMismatch bool, channelIDs *[]string, testDuration int64) (successResult bool, resultStr string) {
        ...
        for c := 0; c < numChannels; c++ {
                ...
                for ord := 0; ord < numOrdsToWatch; ord++ {
                        recvBlocks := blockRecv[ord][c] - 1
                        if expectedBlocksOnChan[c] > recvBlocks { // ignore genesis block
                                successResult = false
                                passFailStr = "FAILED"
                                ...
                                if debugflag1 { logger(fmt.Sprintf("GOOD expected block count %d, received blocks are %d on ordIndx=%d channelIDs[%d]=%s chanSentTxTotal=%d BatchSize=%d", expectedBlocksOnChan[c], recvBlocks, ord, c, (*channelIDs)[c], chanSentTotal, batchSize)) }
                        }
                }
        }
...
func ote( testname string, txs int64, chans int, orderers int, ordType string, kbs int, oteSpy int, pPerCh int, payload int) (passed bool, resultSummary string) {
        ...
        logger(fmt.Sprintf("==========\n========== OTE testname=%s TX=%d Channels=%d Orderers=%d ordererType=%s kafka-brokers=%d oteSpy=%s producersPerCh=%d, payload=%d", testname, txs, chans, orderers, ordType, kbs, spyStatus(oteSpy), pPerCh, payload))

        extraDataSend(payload)   //create the extra payload
