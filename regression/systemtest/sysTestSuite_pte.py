...
class System_Tests_Kafka_Couchdb_TLS(unittest.TestCase):
    ...
    def test_09samplecc_go_98MB_TX(self):

        # Run the test scenario: Execute invokes and query tests.
        returncode = subprocess.call("./operator.sh -t samplecc_go_98MB_TX", cwd=k8s_testsuite, shell=True)
        self.assertEqual(returncode, 0, msg=testScriptFailed)

        # check the counts
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 1 received 1\" samplecc_go_98MB_i_pteReport.txt | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)

class System_Tests_kafka2raft_migration(unittest.TestCase):

    # migrate from kafka to etcdraft network
    # Migrate all channels specified in the networkspec file
    def test_01raftMigration(self):
    ...
    def test_02samplecc_orgAnchor_2chan(self):
...
class System_Tests_Raft_Couchdb_Mutual(unittest.TestCase):
    ...
    def test_08samplecc_go_50MB_TX(self):
        ...
        # check the counts
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 1 received 1\" samplecc_go_50MB_i_pteReport.txt | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)
...
class System_Tests_Kafka_Leveldb_NOTLS(unittest.TestCase):
    ...
    def test_08samplecc_go_98MB_TX(self):
        ...
        # check the counts
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 2 received 2\" samplecc_go_98MB_i_pteReport.txt | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)
