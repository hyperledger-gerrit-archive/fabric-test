...
class Perf_Stress_CouchDB(unittest.TestCase):
    ...
    def test_FAB8192_4i_marbles_FAB8199_4q_FAB8200_4q_FAB8201_4q(self):
        ...
        # FAB-8199-4q
        self.assertIn("FAB-8199-4q_", logfilelist)
        ...
        self.assertEqual(int(count.strip()), 1, msg=noTxSummary)
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 4000 received 4000\" FAB-8199-4q-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)

        # FAB-8200-4q
        self.assertIn("FAB-8200-4q_", logfilelist)
        ...
        self.assertEqual(int(count.strip()), 1, msg=noTxSummary)
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 4000 received 4000\" FAB-8200-4q-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)

        # FAB-8201-4q
        self.assertIn("FAB-8201-4q_", logfilelist)
        ...
        self.assertEqual(int(count.strip()), 1, msg=noTxSummary)
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 4000 received 4000\" FAB-8201-4q-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)
...
class Perf_Stress_LevelDB(unittest.TestCase):
    ...
    def test_FAB7329_4i_channel_events(self):
        ...
        FAB-7329 channel events, 1 ch NodeJS cc, 4 thrds x 1000
        '''
        ...
        # check the counts
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 4000 received 4000\" FAB-7329-4i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)
        ...
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall event: received 4000 timeout 0 unreceived 0\" FAB-7329-4i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=eventReceiveFailure)
    ...
    def test_FAB7333_4i_filtered_block_events(self):
        ...
        FAB-7333 filtered block events, 1 ch NodeJS cc, 4 thrds x 1000
        '''
        ...
        # check the counts
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 4000 received 4000\" FAB-7333-4i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)
        ...
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall event: received 4000 timeout 0 unreceived 0\" FAB-7333-4i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=eventReceiveFailure)
    ...
    def test_FAB7647_1i_latency(self):
        ...
        FAB-7647-1i.sh latency for single thread, 1 transaction at a time sequentially
        '''
        ...
        # check the counts
        count = subprocess.check_output(
                "grep \"LATENCY INVOKE Overall transactions: sent 1000 received 1000\" FAB-7647-1i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)
        ...
        count = subprocess.check_output(
                "grep \"LATENCY INVOKE Overall event: received 1000 timeout 0 unreceived 0\" FAB-7647-1i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=eventReceiveFailure)
        ...
        # if we change PTE behavior to "send and forget", then we would have to tweak this
        # testcase and do some coding to support a latency transMode for Query transactions.
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 1000 received 1000\" FAB-7647-1i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)
