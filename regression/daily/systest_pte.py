...
class Perf_Stress_CouchDB(unittest.TestCase):
    ...
    def test_FAB8192_4i_marbles_FAB8199_4q_FAB8200_4q_FAB8201_4q(self):
        ...
        Launch standard network with couchdb using 1 channel and marbles02 cc.
            FAB-8192-4i 4 threads x 10000 invokes (initMarble)
        ...
        self.assertEqual(int(count.strip()), 1, msg=noTxSummary)
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall transactions: sent 40000 received 40000\" FAB-8192-4i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=invokeFailure)
        count = subprocess.check_output(
        ...
        self.assertEqual(int(count.strip()), 1, msg=invokeSendFailure)
        count = subprocess.check_output(
                "grep \"CONSTANT INVOKE Overall event: received 40000 timeout 0 unreceived 0\" FAB-8192-4i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=eventReceiveFailure)
        ...
        self.assertEqual(int(count.strip()), 1, msg=noTxSummary)
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 40000 received 40000\" FAB-8199-4q-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)
        ...
        self.assertEqual(int(count.strip()), 1, msg=noTxSummary)
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 40000 received 40000\" FAB-8200-4q-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)
        ...
        self.assertEqual(int(count.strip()), 1, msg=noTxSummary)
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 40000 received 40000\" FAB-8201-4q-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)
...
class Perf_Stress_LevelDB(unittest.TestCase):
    ...
    def test_FAB7647_1i_latency(self):
        ...
        count = subprocess.check_output(
                "grep \"CONSTANT QUERY Overall transactions: sent 10000 received 10000\" FAB-7647-1i-pteReport.log | wc -l",
                cwd=logs_directory, shell=True)
        self.assertEqual(int(count.strip()), 1, msg=queryCountFailure)
