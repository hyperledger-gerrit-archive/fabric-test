...
var _ = AfterSuite(func(){
	...
	if containerList != "" {
		...
		networkclient.ExecuteCommand("docker", containerArgs, false);
	}
	ccimagesList := []string{"images", "--filter=reference='dev*'", "-q"}
	images, _ := networkclient.ExecuteCommand("docker", ccimagesList, false);
	if images != "" {
		list := strings.Split(images, "\n")
		imageArgs := []string{"rmi", "-f", images}
		imageArgs = append(imageArgs, list...)
		networkclient.ExecuteCommand("docker", imageArgs, false);
	}
})
