...
#   - git-logs                 - displays the last two commits
...
.PHONY: ci-smoke
ci-smoke: fabric pull-images pull-binaries pull-thirdparty-images build-fabric-ca build-sdk-wrapper smoke-tests
...
.PHONY: jenkins-info
jenkins-info:
	cd $(HYPERLEDGER_DIR)/fabric-test/scripts/Jenkins_Scripts && ./CI_Script.sh --env_Info

.PHONY: jenkins-clean
jenkins-clean:
	cd $(HYPERLEDGER_DIR)/fabric-test/scripts/Jenkins_Scripts && ./CI_Script.sh --clean_Environment
