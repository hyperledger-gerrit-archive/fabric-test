...
PTE_TAG = $(DOCKER_NS)/fabric-pte
TEST_VIEWER_TAG = $(DOCKER_NS)/fabric-testviewer
...
pte:
	...
	docker tag $(PTE_TAG) $(PTE_TAG):$(PROJECT_VERSION)

.PHONY: test-viewer
test-viewer:
	docker build -t $(TEST_VIEWER_TAG) tools/Testviewer
	docker tag $(TEST_VIEWER_TAG) $(TEST_VIEWER_TAG):$(PROJECT_VERSION)
