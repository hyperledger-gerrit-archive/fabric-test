...
def before_scenario(context, scenario):
    ...
    if 'generateDocs' in scenario.tags:
        DocumentGenerator(contextHelper=contextHelper)
...
def after_scenario(context, scenario):
    ...
    if "compose_containers" in context:
        if get_logs.lower() == "force" or (scenario.status == "failed" and get_logs.lower() == "y"):
            print("Scenario {0} {1}. Getting container logs".format(scenario.name, scenario.status))
    ...
    if os.path.exists("../chaincodes/shim-vendored/plob-1.0-shim/chaincode/vendor"):
