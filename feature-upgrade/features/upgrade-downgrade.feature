...
Feature: Bootstrap
  ...
  @doNotDecompose
  @generateDocs
  Scenario Outline: Bootstrap a development network with 4 peers (2 orgs)  and 1 orderer (1 org), each having a single independent root of trust (No fabric-ca, just openssl) and then upgrade
...
#    Then the orderers will reject the update
    ...
    # Do some invoke to verify a peer
    When user "dev0Org0" using cert alias "consortium1-cert" creates a proposal "invokeProposalPostUpgrade" for channel "com.acme.blockchain.jdoe.channel1" using chaincode spec "invocationSpec1"

    And user "dev0Org0" using cert alias "consortium1-cert" sends proposal "invokeProposalPostUpgrade" to endorsers with timeout of "30" seconds with proposal responses "invokeProposalPostUpgradeResponses":
      | Endorser |
      | peer0    |
      | peer2    |

    Then user "dev0Org0" expects proposal responses "invokeProposalPostUpgradeResponses" with status "200" from endorsers:
      | Endorser |
      | peer0    |
      | peer2    |

    And user "dev0Org0" expects proposal responses "invokeProposalPostUpgradeResponses" each have the same value from endorsers:
      | Endorser |
      | peer0    |
      | peer2    |

    When the user "dev0Org0" creates transaction "invokeTxPostUpgrade" from proposal "invokeProposalPostUpgrade" and proposal responses "invokeProposalPostUpgradeResponses" for channel "com.acme.blockchain.jdoe.channel1"

    And the user "dev0Org0" broadcasts transaction "invokeTxPostUpgrade" to orderer "<orderer0>"
    ...
    # Same thing with Peer Admins for 1 peer per org
    ...
    # Do some invoke to verify upgraded peers
...
########################################################################
    And user "dev0Org0" using cert alias "consortium1-cert" connects to deliver function on node "<orderer0>" using port "7050"
    And user "dev0Org0" retrieves the latest config update "latestChannelConfigAfterDowngradeOfOrderersFromOrderer0" from orderer "<orderer0>" for channel "com.acme.blockchain.jdoe.channel1"
...
########################################################################
    ...
    And user "dev0Org0" using cert alias "consortium1-cert" creates a proposal "invokeProposalPostDowngrade" for channel "com.acme.blockchain.jdoe.channel1" using chaincode spec "invocationSpec2"
    ...
    And the user "dev0Org0" broadcasts transaction "invokeTxPostDowngrade" to orderer "<orderer0>"
