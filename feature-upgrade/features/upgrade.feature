...
#     | peer3    |
    ...
    ### NOTE: I am guessing Jeff put this "disconnect" step here only because
    ### it might be needed to allow this test framework to work cleanly.
    ### It seems like it should not be required in real world...
...
###
    ...
    And user "dev0Org0" using cert alias "consortium1-cert" sends proposal "invokeProposalAfterSomePeersUpversioned" to endorsers with timeout of "30" seconds with proposal responses "invokeProposalResponsesAfterSomePeersUpversioned":
      | Endorser |
      | peer0    |
      | peer2    |
...
#     | peer3    |
    ...
    # NOTE: Currently 2 step process as the ordere will fixup the
    # /Channel group mod_policy after this upgrade of '/Channel/Orderer'
    #
    ###########################################################################
    ###########################################################################
...
#      | configAdminOrdererOrg1 | config-admin-cert |
    ...
    # Clean up chaincode docker images while those old-version peers are down
    Given all peer admins remove existing chaincode docker images
    ...
    And user "dev0Org0" using cert alias "consortium1-cert" sends proposal "invokeProposalAfterAllPeersUpversioned" to endorsers with timeout of "30" seconds with proposal responses "invokeProposalResponsesAfterAllPeersUpversioned":
      | Endorser |
      | peer0    |
      | peer2    |
...
#      | dc-base.yml  dc-peer-couchdb.yml                      | 10               | solo          | 2                | 2                 | orderer0 | orderer0 | orderer0 |                       | 2                      | latest                | 2                   | latest             |
      | dc-base.yml  dc-orderer-kafka.yml | 60               | kafka         | 10               | 5                 | orderer0 | orderer1 | orderer2 |                       | 2                      | x86_64-1.0.4      | latest                | 30                   | latest             | 30                            |
