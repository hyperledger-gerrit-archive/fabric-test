...
#    And I wait "<SystemUpWaitTime>" seconds
    ...
    # For now, we must uncomment this next line if testing solo, since we already reconnected and must disconnect from orderer0 again. (Ideally, we should just enhance the disconnect step, so we could specify a single orderer.)
    #Given all users disconnect from orderers
    Given all orderer admins agree to upgrade
    And we "stop" service "<orderer1>"
...
#     | peer3    |
    ...
    # FabricBaseVersion normally is 'x86_64-1.0.4'. To upgrade from other versions, remember to docker pull the images as instructed in the README.md.
