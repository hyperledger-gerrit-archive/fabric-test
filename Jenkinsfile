...
node ('hyp-x') { // trigger build on x86_64 node
 timestamps {
    try {
     ...
     env.VERSION = sh(returnStdout: true, script: 'curl -O https://raw.githubusercontent.com/hyperledger/fabric/master/ci.properties && cat ci.properties | grep "GO_VER" | cut -d "=" -f2').trim()
     env.ARCH = "amd64"
          ...
          try {
             if (jobname == "patchset")  {
                   ...
                   checkout([
                       ...
                       extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: '$GIT_BASE'], [$class: 'CheckoutOption', timeout: 10]],
                   ...
                   checkout([
                       ...
                       extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: '$GIT_BASE']],
