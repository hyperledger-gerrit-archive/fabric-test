...
node ('hyp-x') { // trigger build on x86_64 node
 timestamps {
    try {
     ...
     env.GO_VER = sh(returnStdout: true, script: 'curl -O https://raw.githubusercontent.com/hyperledger/fabric/release-1.4/ci.properties && cat ci.properties | grep "GO_VER" | cut -d "=" -f2').trim()
     env.ARCH = "amd64"
...
// clean environment and get env data
      stage("Clean Environment - Get Env Info") {
          wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'xterm']) {
           try {
                 dir("${ROOTDIR}/$PROJECT_DIR/fabric-test/scripts/Jenkins_Scripts") {
      ...
      stage("Setup Environment") {
         wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'xterm']) {
           try {
                 ...
                 sh 'make git-init git-latest pre-setup fabric'
                 }
               }
           catch (err) {
