...
COMMIT_VERSION=29a7d95
OS_VER=$(uname -s|tr '[:upper:]' '[:lower:]')
echo
rm -rf .build && mkdir -p .build && cd .build
curl https://nexus.hyperledger.org/content/repositories/releases/org/hyperledger/fabric/hyperledger-fabric-stable/$OS_VER-$ARCH.$STABLE_VERSION-$COMMIT_VERSION/hyperledger-fabric-stable-$OS_VER-$ARCH.$STABLE_VERSION-$COMMIT_VERSION.tar.gz | tar xz
