...
dockerThirdParty() {
  for IMAGE in kafka zookeeper couchdb; do
    ...
    docker pull $ORG_NAME-$IMAGE:$ARCH-0.4.14
    if [ $? != 0 ]; then
       echo  "FAILED: Docker Pull Failed on $IMAGE"
       exit 1
    fi
    docker tag $ORG_NAME-$IMAGE:$ARCH-0.4.14 $ORG_NAME-$IMAGE:latest
  done
}
...
fabric-sdk-node)
  ...
  dockerTag peer orderer ccenv tools ca ca-fvt javaenv
  ;;
fabric-sdk-java)
...
fabric-javaenv)
  ...
  dockerTag peer orderer ccenv tools ca ca-fvt
  ;;
third-party)
