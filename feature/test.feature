...
Scenario Outline: FAB-1111: Test with Fabric-CA
  ...
  Then a user receives a success response of 990
    ...
    When a user "adnan" invokes on the chaincode with args ["invoke", "a", "b", "10"] on peer0.org2.example.com
    And I wait "5" seconds
    #And a user "latitia" queries on the channel "chn2" using chaincode named "cc2" with args ["get", "a"]
    And a user "latitia" queries on the chaincode with args ["query", "a"]
...
Examples:
    ...
    | kafka | leveldb  |    CLI     |        ../../fabric-test/chaincodes/example02/node                |   NODE   |
