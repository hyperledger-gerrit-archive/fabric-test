...
Scenario Outline: [FAB-4683] [FAB-4684] [FAB-4685] In leader-selection setup, leader peer goes down by <takeDownType> for at least <minDownDuration> seconds, comes back up and catches up eventually
  ...
  # Now do three invoke-query pairs
  When a user invokes on the chaincode named "mycc" with args ["invoke","a","b","20"] on "peer0.org2.example.com"
  And I wait "5" seconds
  ...
  When a user invokes on the chaincode named "mycc" with args ["invoke","a","b","30"] on "peer0.org2.example.com"
  And I wait "5" seconds
  When a user queries on the chaincode named "mycc" with args ["query","a"] on "peer0.org2.example.com"
  Then a user receives a success response of 940 from "peer0.org2.example.com"
  When a user invokes on the chaincode named "mycc" with args ["invoke","a","b","40"] on "peer0.org2.example.com"
  And I wait "5" seconds
  When a user queries on the chaincode named "mycc" with args ["query","a"] on "peer0.org2.example.com"
