...
Scenario Outline: [FAB-4676] [FAB-4677] [FAB-4678] all the peers of an organization goes down temporarily by <takeDownType>, catches up 
  ...
  And I wait "5" seconds
  ## Now do 3 invoke-queries in leader peer from org1
  When a user invokes on the chaincode named "mycc" with args ["invoke","a","b","10"]
  And I wait "5" seconds
  ...
  Then a user receives a success response of 920 from the initial leader peer of "org2"
  When a user queries on the chaincode named "mycc" with args ["query","a"] on the initial non-leader peer of "org2" 
  Then a user receives a success response of 920 from the initial non-leader peer of "org2" 
