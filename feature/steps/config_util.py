...
def getCurrentConfig(context, channel):
    ...
    subprocess.check_output(["configtxlator", "proto_decode", "--input", block, "--type", "common.Block", "--output", "configStr.pb"], cwd=testConfigs , env=updated_env)
    with open("{0}/configStr.pb".format(testConfigs), "r") as fd:
...
def configUpdate(context, config, group, channel):
    ...
    subprocess.check_output(["configtxlator", "proto_decode", "--input", "update.pb", "--type", "common.ConfigUpdate", "--output", "configStr.pb"],
                            cwd=testConfigs,
                            env=updated_env)
    with open("{0}/configStr.pb".format(testConfigs), "r") as fd:
        config = json.loads(fd.read())
