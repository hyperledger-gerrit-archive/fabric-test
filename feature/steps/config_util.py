...
#    cert_base = "{0}/peerOrganizations/{1}/msp".format(configDir, mspID)
#    with open("{0}/admincerts/Admin@{1}-cert.pem".format(cert_base, mspID), "rb") as fd:
#        #admin_cert = unicode(base64.b64encode(fd.read()+'\n'))
#        admin_cert = marshal.loads(fd.read())
#    with open("{0}/cacerts/ca.{1}-cert.pem".format(cert_base, mspID), "rb") as fd:
#        #root_cert = unicode(base64.b64encode(fd.read()+'\n'))
#        root_cert = marshal.loads(fd.read())
#    with open("{0}/tlscacerts/tlsca.{1}-cert.pem".format(cert_base, mspID), "rb") as fd:
#        #tls_root_cert = unicode(base64.b64encode(fd.read()+'\n'))
#        tls_root_cert = marshal.loads(fd.read())
#
...
#                                     "version": "0",
#                                     "policy": {"type": 1,
#                                                "value": {"identities": [{"principal": {"msp_identifier": mspID,
