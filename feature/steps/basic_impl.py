...
@when(u'an admin adds a new organization to the {channelName} channel')
def step_impl(context, channelName):
    ...
    config_util.buildCryptoFile(context, 1, 2, 0, 2, orgName="org3.example.com")
    config_util.generateCrypto(context, "./configs/{0}/crypto.yaml".format(context.projectName))
    config_util.generateCryptoDir(context, 1, 2, 0, 2, tlsExist=context.tlsEnabled, orgName="org3.example.com")

    # Format the args for adding a new org org3.example.com (MSPID: Org3ExampleCom)

    update_impl(context, 'peer', channelName, args, userName='Admin')

@when(u'an admin adds a new organization named {orgName} to the {channelName} channel')
def step_impl(context, orgName, channelName):
    ...
    # Format the args for adding a new org with the orgName

    update_impl(context, 'peer', channelName, args, userName='Admin')

@when(u'an admin removes a organization named {orgName} from the {channelName} channel')
def step_impl(context, orgName, channelName):
