...
def bootstrapped_impl(context, networkType):
    assert networkType in config_util.ORDERER_TYPES, "Unknown network type '%s'" % networkType
    ...
    context.ordererProfile = config_util.PROFILE_TYPES.get(networkType, "SampleInsecureSolo")
...
@when(u'I start a fabric network of type {networkType}')
def start_network_impl(context, networkType):
    assert networkType in config_util.ORDERER_TYPES, "Unknown network type '%s'" % networkType
    curpath = os.path.realpath('.')
    context.composeFile = "%s/docker-compose/docker-compose-%s.yml" % (curpath, networkType)
    assert os.path.exists(context.composeFile), "The docker compose file does not exist: {0}".format(context.composeFile)
