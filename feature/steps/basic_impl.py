...
def bootstrapped_impl(context, ordererType, database, tlsEnabled=False, timeout=300, fca=False, ouEnabled=False):
    ...
    elif not ouEnabled and not fca:
    ...
    elif not ouEnabled and fca:
        config_util.generateConfigForCA(context, channelID, config_util.CHANNEL_PROFILE, context.ordererProfile)
    else:
        print("WARNING! There may be an error around using Fabric-CA with OU enabled security")
...
@when(u'an admin updates the channel with {args}')
...
def update_impl(context, component, channelName, args, userName='Admin'):
    ...
    block_filename = config_util.configUpdate(context, args, "Application", channelName)

    # have other peers sign updated channel block
    context.interface.sign_channel(context, peers, block_filename, user="Admin")

    # send signed, updated block to the channel
    context.interface.update_channel(context, peers, channelName, "orderer0.example.com", block_filename, user="Admin")
