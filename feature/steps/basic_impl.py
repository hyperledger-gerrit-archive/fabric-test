...
@when(u'a user defines a couchDB index named {indexName} from {docName} documents containing the field "{field}" to the chaincode at path "{path}"')
def step_impl(context, indexName, docName, field, path):
    database_util.generateIndex(indexName, docName, field, path)
