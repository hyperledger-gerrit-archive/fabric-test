...
@when(u'a user queries on the chaincode named "{name}" with args {args} on the initial elected-leader peer of "{org}"')
def step_impl(context, name, args, org):
    query_impl(context, endorser_util.TEST_CHANNEL_ID, name, args, endorser_util.get_initial_elected_leader(context, org))
...
@when(u'a user invokes on the chaincode named "{name}" with args {args} on the initial elected-leader peer of "{org}"')
def step_impl(context, name, args, org):
    invokes_impl(context, 1, endorser_util.TEST_CHANNEL_ID, name, args, endorser_util.get_initial_elected_leader(context, org))
...
@when(u'a user invokes on the chaincode named "{name}" with args {args} on {component}')
def step_impl(context, name, args, component):
    invokes_impl(context, 1, endorser_util.TEST_CHANNEL_ID, name, args, component)
...
@then(u'a user receives {status} response of {response} from the initial elected-leader peer of "{org}"')
def step_impl(context, response, org, status):
    expected_impl(context, response, endorser_util.get_initial_elected_leader(context, org))
