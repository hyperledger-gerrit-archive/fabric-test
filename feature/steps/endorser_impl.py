...
def install_impl(context, path, args, name, peer, language, version, username="Admin"):
...
def step_impl(context, path, args, name, language, version):
    install_impl(context, path, args, name, "peer0.org1.example.com", language, version)

@when(u'a user installs chaincode at path "{path}" as version "{version}" with args {args} with name "{name}" to "{peer}"')
def step_impl(context, path, args, name, peer, version):
    install_impl(context, path, args, name, peers, "GOLANG", version)
...
def step_impl(context, path, args, name, version):
    install_impl(context, path, args, name, "peer0.org1.example.com", "GOLANG", version)
...
def step_impl(context, path, args, name, peer):
    install_impl(context, path, args, name, peer, "GOLANG", version)
