...
@when(u'an admin installs chaincode at path "{path}" as version "{version}" with args {args} with name "{name}" to "{peer}"')
...
@when(u'an admin installs chaincode at path "{path}" as version "{version}" with args {args} with name "{name}" on all peers')
def step_impl(context, path, args, name, version, username="Admin"):
    peers = context.interface.get_peers(context)
    context.interface.pre_deploy_chaincode(context, path, args, name, "GOLANG", version=version)
    context.interface.install_chaincode(context, peers, "Admin")

@when(u'an admin installs chaincode at path "{path}" as version "{version}" with args {args} on all peers')
