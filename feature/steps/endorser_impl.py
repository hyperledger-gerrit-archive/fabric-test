...
def step_impl(context, name, args, peer, targs):
    ...
    query_impl(context, context.interface.TEST_CHANNEL_ID, name, args, peer, '"{' + generated+ '}"')
...
def step_impl(context, name, args, targs):
    # This is a workaround to allow targs to send a json structure
    generated = targs[2:-2].format(**context.command_result)
    query_impl(context, context.interface.TEST_CHANNEL_ID, name, args, "peer0.org1.example.com", '"{' + generated + '}"')
...
def step_impl(context, name, args, targs, peer):
    # This is a workaround to allow targs to send a json structure
    generated = targs[2:-2].format(**context.command_result)
    invokes_impl(context, 1, context.interface.TEST_CHANNEL_ID, name, args, peer, '"{' + generated + '}"')
...
@when(u'a user invokes on the chaincode named "{name}" with generated args {args} on peer "{peer}"')
def generated_invoke_impl(context, name, args, peer):
    chaincode = {"args": args.format(**context.command_result),
                 "name": name}
    context.result = context.interface.invoke_chaincode(context, chaincode, ["orderer0.example.com"], peer, context.interface.TEST_CHANNEL_ID)

@when(u'a user invokes on the chaincode named "{name}" with generated args {args}')
def step_impl(context, name, args):
    generated_invoke_impl(context, name, args, "peer0.org1.example.com")
