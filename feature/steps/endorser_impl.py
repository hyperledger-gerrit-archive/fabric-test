...
@then(u'a user receives a response with {valueType} value from "{peer}"')
def set_response_impl(context, valueType, peer):
    ...
    if valueType == "length":
        ...
        assert context.result[peer] == context.payload["payload"], \
...
@then(u'a user receives a response with {valueType} value')
