...
@when(u'a user deploys chaincode at path "{path}" with args {args} with name "{name}" with language "{language}" to "{containerName}" on channel "{channel}" within {timeout:d} seconds')
def deploy_impl(context, path, args, name, language, containerName, channel, timeout=120):
    # Be sure there is a transaction block for this channel
    ...
    chaincode_container = "{0}-{1}-{2}-0".format(context.projectName, containerName, chaincode['name'])
...
def step_impl(context, path, args, name, language, containerName, channel):
    deploy_impl(context, path, args, name, language, containerName, channel, 120)

@when(u'a user deploys chaincode at path "{path}" with args {args} with name "{name}" to "{containerName}" on channel "{channel}" within {timeout:d} seconds')
def step_impl(context, path, args, name, containerName, channel, timeout):
    deploy_impl(context, path, args, name, "GOLANG", containerName, channel, timeout)
