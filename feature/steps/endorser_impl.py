...
def step_impl(context, name, fileName, policy):
    peers = context.interface.get_peers(context)
    context.chaincode['name'] = name
    #context.result = context.interface.approve_chaincode(context, peers, user="Admin", policy=policy, collections="/var/hyperledger/configs/{}/META-INF/statedb/couchdb/indexes/index_behave_test.json".format(context.projectName), initRequired=False)
    context.result = context.interface.approve_chaincode(context, peers, user="Admin", policy=policy, collections="/var/hyperledger/configs/{0}/{1}".format(context.projectName, fileName), initRequired=False)
...
def step_impl(context, version, policy, peer):
    context.chaincode['version'] = version
    context.result = context.interface.approve_chaincode(context, [peer], user="Admin", upgrade=True, policy=policy, collections=None)
...
def step_impl(context, fileName):
    #context.result = context.interface.commit_chaincode(context, "peer0.org1.example.com", user="Admin", policy=None, collections="/var/hyperledger/configs/{}/META-INF/statedb/couchdb/indexes/index_behave_test.json".format(context.projectName), initRequired=False)
