...
@given(u'we have a fabric config file')
def step_impl(context):
    config_util.generateCrypto(context)
    config_util.setupConfigs(context, TEST_CHANNEL_ID)

@given(u'we have a crypto config file with {numOrgs} orgs, {numPeers} peers, {numOrderers} orderers, and {numUsers} users')
...
@when(u'the network is bootstrapped for an orderer of type {networkType}')
def ordererBootstrap_impl(context, networkType):
    profile = config_util.PROFILE_TYPES[networkType]
...
@then(u'crypto directories are generated containing certificates for {numOrgs}, {numPeers}, {numOrderers}, and {numUsers}')
def step_impl(context, numOrgs, numPeers, numOrderers, numUsers):
    config_util.generateCryptoDir(context, numOrgs, numPeers, numOrderers, numUsers, tlsExist=False)

@then(u'crypto directories are generated containing tls certificates for {numOrgs}, {numPeers}, {numOrderers}, and {numUsers}')
def step_impl(context, numOrgs, numPeers, numOrderers, numUsers):
...
def step_impl(context, fileName, value):
    with open("./configs/{0}/{1}".format(context.projectName, fileName), 'r') as blockFile:
