...
class SDKInterface(InterfaceBase):
    ...
    def initializeNode(self):
        #TODO: Revisit
        npminstall =  subprocess.check_call(["npm", "install"], env=os.environ, cwd="./")
        #npminstall =  subprocess.check_call(["npm", "install", "fabric-client"], env=os.environ, cwd="./sdk/node")
        with open("./sdk/node/invoke-transaction.js", "r") as fd:
            invoke_text = fd.read()
    ...
    def invoke_chaincode(self, context, chaincode, orderer, peer, channelId=TEST_CHANNEL_ID):
        configDir = "../configs/{0}".format(context.composition.projectName)
        args = chaincode.get('args', '[]').replace('"', r'\"')
        peerParts = peer.split('.')
        org = '.'.join(peerParts[1:])
        #TODO: lots of cleanup here
        chaincode["args"] = ["a","b","10"]
        chaincode["fcn"] = 'invoke'
        chaincode["name"] = ''
        chaincode["chaincodeId"] = 'mycc'
        chaincode["channelId"] = channelId
        #invoke('Admin@org1.example.com', 'org1.example.com', {"args": ["a","b","10"], "fcn":"invoke", "channelId": "behavesystest", "chaincodeId": "mycc"}, ["peer0.org1.example.com"], "orderer0.example.com", "/Users/ratnakar/workspace/go/src/github.com/hyperledger/fabric-test/feature/configs/6b068d85adf711e7b02780e65025f612/network-config.json");
        self.invoke_func.call("invoke", "User1@{}".format(org), org, chaincode, [peer], orderer, self.networkConfigFile)


#    def query_chaincode(self, context, chaincode, peer, channelId):
#        self.query_func.call("query", "User1", org, chaincode, peer, self.networkConfigFile)
