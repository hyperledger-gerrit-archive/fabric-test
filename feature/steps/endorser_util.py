...
class InterfaceBase:
    ...
    def deploy_chaincode(self, context, chaincode, containers, channelId=TEST_CHANNEL_ID):
        ...
        self.create_channel(context, containers, orderers, channelId)
        #self.fetch_channel(context, peers, orderers, channelId)
        self.join_channel(context, peers, orderers, channelId)
...
class ToolInterface(InterfaceBase):
    ...
    def create_channel(self, context, containers, orderers, channelId):
        ordererList = [context.networkInfo["nodes"][o]["nodeName"][-1] for o in orderers]
        peer_name = context.networkInfo["nodes"]["peer0.org1.example.com"]["nodeName"]
    ...
    def join_channel(self, context, peers, orderers, channelId):
        results = {}
        for peer in peers:
            peer_name = context.networkInfo["nodes"][peer]["nodeName"]
    ...
    def invoke_chaincode(self, context, chaincode, orderers, peer, channelId):
        args = json.loads(chaincode["args"])
        peer_name = context.networkInfo["nodes"][peer]["nodeName"]
...
class CLIInterface(InterfaceBase):
    ...
    def get_env_vars(self, context, peer="peer0.org1.example.com"):
        configDir = "/var/hyperledger/configs/{0}".format(context.composition.projectName)
    ...
    def create_channel(self, context, containers, orderers, channelId=TEST_CHANNEL_ID):
        configDir = "/var/hyperledger/configs/{0}".format(context.composition.projectName)
    ...
    def fetch_channel(self, context, peers, orderers, channelId=TEST_CHANNEL_ID):
        configDir = "/var/hyperledger/configs/{0}".format(context.composition.projectName)
        for peer in peers:
            peerParts = peer.split('.')
            org = '.'.join(peerParts[1:])
