...
class SDKInterface(InterfaceBase):
    ...
    def initializeNode(self):
        npminstall =  subprocess.check_call(["npm", "install", "--silent"], env=os.environ)
        #npminstall =  subprocess.check_call(["npm", "install", "fabric-client", "--silent"], env=os.environ)
    ...
    def invoke_chaincode(self, context, chaincode, orderer, peer, channelId=TEST_CHANNEL_ID):
        ...
        print("reformatted chaincode: {}".format(chaincode))

        peerParts = peer.split('.')
        org = '.'.join(peerParts[1:])
        orgName = org.title().replace('.', '')
        #invoke_func.call("invoke", "Admin@{}".format(org), org, chaincode, peer, orderer, network_file_contents)
        #self.invoke_func.call("invoke", "Admin@{}".format(org), org, chaincode, peer, orderer, self.networkConfigFile)
        self.invoke_func.call("invoke", "User1@{}".format(org), orgName, reformatted, [peer], orderer, self.networkConfigFile)

#    def query_chaincode(self, context, chaincode, peer, channelId):
#        reformatted = self.reformat_chaincode(chaincode)
#        peerParts = peer.split('.')
#        org = '.'.join(peerParts[1:])
#        orgName = org.title().replace('.', '')
#        self.query_func.call("query", "User1@{}".format(org), orgName, reformatted, [peer], self.networkConfigFile)
