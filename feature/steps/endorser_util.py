...
class CLIInterface(InterfaceBase):
    ...
    def enrollCAadmin(self, context, node, admin="Admin", passwd="adminpw"):
        ...
        url = "{3}://{0}:{2}@ca.{1}:7054".format(admin, org, passwd, proto)
        output = context.composition.docker_exec(["fabric-ca-client enroll -d -u {0} -M /var/hyperledger/msp --caname ca.{1} --csr.hosts localhost,ca.{1} --csr.cn ca.{1} --tls.certfiles /var/hyperledger/msp/cacerts/ca.{1}-cert.pem".format(url, org)], [node])
    ...
    def enrollUser(self, context, user, org, passwd, enrollType, component, orgType):
        ...
        if orgType == "peerOrganizations":
            command = "fabric-ca-client enroll -d --enrollment.profile tls -u {8}://{0}:{1}@{3}:7054 -M /var/hyperledger/users/{0}@{2}/tls --csr.hosts localhost,{4} --enrollment.type {5} --tls.certfiles /var/hyperledger/configs/{6}/{7}/{2}/ca/ca.{2}-cert.pem".format(user, passwd, org, fca, component, enrollType, context.projectName, orgType, proto)
            #command = "fabric-ca-client enroll -d --enrollment.profile tls -u https://{0}:{1}@{3}:7054 -M /var/hyperledger/users/{0}@{2}/tls --enrollment.type {5} --tls.certfiles /var/hyperledger/configs/{6}/{7}/{2}/ca/ca.{2}-cert.pem".format(user, passwd, org, fca, component, enrollType, context.projectName, orgType)
        else:
            command = "fabric-ca-client enroll -d --enrollment.profile tls -u {7}://{0}:{1}@{3}:7054 -M /var/hyperledger/users/{0}@{2}/tls --csr.hosts localhost,{4} --enrollment.type {5} --tls.certfiles /var/hyperledger/configs/{6}/{2}/ca/ca.{2}-cert.pem".format(user, passwd, org, fca, component, enrollType, orgType, proto)
            #command = "fabric-ca-client enroll -d --enrollment.profile tls -u https://{0}:{1}@{3}:7054 -M /var/hyperledger/users/{0}@{2}/tls --enrollment.type {5} --tls.certfiles /var/hyperledger/configs/{6}/{2}/ca/ca.{2}-cert.pem".format(user, passwd, org, fca, component, enrollType, orgType)
        output = context.composition.docker_exec([command], [component])
