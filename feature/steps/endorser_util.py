...
def get_initial_elected_leader(context, org):
    if hasattr(context, 'initial_elected_leader')==False:
        context.initial_elected_leader={}
    if org not in context.initial_elected_leader:
        for container in context.composition.collectServiceNames():
            if (container.startswith("peer") and (org in container) and
                search_log(container, "Becoming a leader")):
                context.initial_elected_leader[org]=container
                print("initial elected leader is "+context.initial_elected_leader[org])
                return context.initial_elected_leader[org]
        assert org in context.initial_elected_leader.keys(), "Error: No gossip-leader found by looking at the logs, for "+org
    return context.initial_elected_leader[org]
