...
class InterfaceBase:
    ...
    def invoke_chaincode(self, context, chaincode, orderer, peer, channelId, targs="", opts={}, user="User1"):
        # targs, opts and user are optional parameters with defaults set if they are not included
        return self.cli.invoke_chaincode(context, chaincode, orderer, peer, channelId, targs, opts, user)
...
class NodeSDKInterface(SDKInterface):
    ...
    def query_func(self, chaincode, channelId, user, org, peers, opts):
        ...
        assert match, "No matching response from query result"
        return match[0]
...
class CLIInterface(InterfaceBase):
    ...
    def invoke_chaincode(self, context, chaincode, orderer, peer, channelId=TEST_CHANNEL_ID, targs="", opts={}, user="User1"):
        # channelId, targs, opts and user are optional parameters with defaults set if they are not included
        configDir = "/var/hyperledger/configs/{0}".format(context.composition.projectName)
