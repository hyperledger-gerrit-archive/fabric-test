...
class InterfaceBase:
    ...
    def instantiate_chaincode(self, context, chaincode, peersuser="Admin"):
        return self.cli.instantiate_chaincode(context, chaincode, peers, user=user)

    def create_channel(self, context, orderer, channelIduser="Admin"):
        return self.cli.create_channel(context, orderer, channelId, user=user)

    def fetch_channel(self, context, peers, orderer, channelIduser="Admin"):
        return self.cli.fetch_channel(context, peers, orderer, channelId, user=user)

    def join_channel(self, context, peers, channelIduser="Admin"):
        return self.cli.join_channel(context, peers, channelId, user=user)

    def invoke_chaincode(self, context, chaincode, orderer, peer, channelIduser="User1"):
        return self.cli.invoke_chaincode(context, chaincode, orderer, peer, channelId, user=user)

    def query_chaincode(self, context, chaincode, peer, channelIduser="User1"):
