...
class CLIInterface(InterfaceBase):
    ...
    def invoke_chaincode(self, context, chaincode, orderers, peer, channelId=TEST_CHANNEL_ID):
        ...
        output = self.retry(output, peer, setup, command)
        return output
    ...
    def query_chaincode(self, context, chaincode, peer, channelId=TEST_CHANNEL_ID):
        ...
        result = self.retry(result, peer, setup, command)
        return result

    def retry(self, output, peer, setup, command):
        if "been successfully instantiated and try again" in output[peer]:
            time.sleep(5)
            print("Received: {0}, Trying again...".format(output[peer]))
            output = context.composition.docker_exec(setup+command, [peer])
        return output
