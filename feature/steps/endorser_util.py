...
class CLIInterface(InterfaceBase):
    ...
    def enrollUser(self, context, user, org, passwd, enrollType, component, orgType):
        ...
        if orgType == "peerOrganizations":
            command = "fabric-ca-client enroll -d --enrollment.profile tls -u https://{0}:{1}@{3}:7054 -M /var/hyperledger/users/{0}@{2}/tls --csr.hosts localhost,{4} --enrollment.type {5} --tls.certfiles /var/hyperledger/configs/{6}/{7}/{2}/ca/ca.{2}-cert.pem".format(user, passwd, org, fca, component, enrollType, context.projectName, orgType)
        else:
            command = "fabric-ca-client enroll -d --enrollment.profile tls -u https://{0}:{1}@{3}:7054 -M /var/hyperledger/users/{0}@{2}/tls --csr.hosts localhost,{4} --enrollment.type {5} --tls.certfiles /var/hyperledger/configs/{6}/{2}/ca/ca.{2}-cert.pem".format(user, passwd, org, fca, component, enrollType, orgType)
        output = context.composition.docker_exec([command], [component])
