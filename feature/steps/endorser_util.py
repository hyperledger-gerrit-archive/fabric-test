...
class CLIInterface(InterfaceBase):
    ...
    def placeCertsInDirStruct(self, context, user, org, peer):
        ...
        output = context.composition.docker_exec(['stat -c "%u %g" /var/hyperledger/users/Admin@{0}'.format(org, context.projectName)], [peer])
        out = output[peer].strip().split(" ")
        print("stat:: {}".format(out))
        output = context.composition.docker_exec(['chown -R {3}:{4} /var/hyperledger/users/{0}@{1}'.format(user, org, context.projectName, out[0], out[1])], [peer])
