...
#            print("user register: {}".format(output))
    ...
    def enrollUsersFabricCA(self, context):
        ...
        for user in context.users.keys():
            ...
            command = "fabric-ca-client enroll -d -u https://{4}@{5}:7054 --enrollment.profile tls --id.name {0} --id.secret {1} --id.affiliation {2} -M {3}/peerOrganizations/{2}/users/{0}@{2}/msp".format(user, passwd, org, configDir, adminUser, fca)
