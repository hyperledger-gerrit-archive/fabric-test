...
Scenario: FAB-8380: Test MSP Identity - - Malicious Peer
  Given the CORE_PEER_TLS_CLIENTAUTHREQUIRED environment variable is "true"
  And the ORDERER_TLS_CLIENTAUTHREQUIRED environment variable is "true"
  And the peer "peer1.org2.example.com" is setup to use a client identity
  ...
  Then the logs on orderer0.example.com contains "Evaluation Failed" within 10 seconds
  And I wait "5" seconds
  When a user queries on the chaincode named "mycc" with args ["query","a"] on "peer0.org1.example.com"
  Then a user receives a success response of 1000
  Then the logs on orderer0.example.com contains "Signature set did not satisfy policy /Channel/Application/Org2ExampleCom/Writers" within 5 seconds
