...
Examples:
    | type  | database | waitSetup |  security   |
    | solo  | leveldb  |     5     |  with tls   |
    | solo  | leveldb  |     5     | without tls |
    | solo  | couchdb  |     5     |  with tls   |
    | solo  | couchdb  |     5     | without tls |
    | kafka | leveldb  |    30     |  with tls   |
    | kafka | leveldb  |    30     | without tls |
    | kafka | couchdb  |    30     |  with tls   |
    | kafka | couchdb  |    30     | without tls |
...
Scenario: LMH-2468: Consensus Testing
    ...
    And I wait "60" seconds
    And a user queries on the chaincode with args ["query","a"]
    Then a user receives a success response of 850
    When a user queries on the chaincode with args ["query","a"] from "peer1.org1.example.com"
    Then a user receives a success response of 850 from "peer1.org1.example.com"
    When a user queries on the chaincode with args ["query","a"] from "peer1.org2.example.com"
    Then a user receives a success response of 850 from "peer1.org2.example.com"
    ...
    Then a user receives a success response of 750 from "peer0.org2.example.com"
