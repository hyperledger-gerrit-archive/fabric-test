...
Scenario: Setting of environment variables
    ...
    And the CORE_LOGGING_GOSSIP environment variable is CRITICAL
    # CORE_PEER_TLS_ENABLED and ORDERER_GENERAL_TLS_ENABLED are also
    # defined (default to false) in file fabric-test/feature/.env.
    # They are set to true in the statement below when we say "with tls".
    ...
    And the CORE_LOGGING_GOSSIP environment variable is CRITICAL on node "peer1.org2.example.com"
    And the CORE_PEER_TLS_ENABLED environment variable is true on node "peer0.org1.example.com"
    And the ORDERER_GENERAL_TLS_ENABLED environment variable is true on node "orderer0.example.com"
