...
Scenario: FAB-4703: FAB-5663, Test chaincode calling chaincode - fabric/examples/chaincode_example04
  ...
  @daily
  Scenario: FAB-4717-Good: FAB-5663, chaincode-to-chaincode testing passing in channel name as a third argument to chaincode_ex05 when cc_05 and cc_02 are on different channels
    Given I have a bootstrapped fabric network of type kafka
    When a user sets up a channel
...
@daily
Scenario: FAB-4717-Error: FAB-5663, chaincode-to-chaincode testing passing in channel name as a third argument to chaincode_ex05 when cc_05 and cc_02 are on different channels
  Given I have a bootstrapped fabric network of type kafka
  When a user sets up a channel
  And a user deploys chaincode at path "github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example05" with args ["init","sum","0"] with name "myex05"
  When a user sets up a channel named "channel2"
...
Scenario Outline: FAB-5790: Test chaincode marbles02 initMarble/readMarble/deleteMarble/getHistoryForMarble/getMarblesByRange in <language> language for <type> orderer
  ...
  When a user invokes  1 faileon the chaincode named "mycc" with args ["initMarble","marble101","red","35","tom"]
  And I wait "3" seconds

  # Test getMarblesByRange
  When a user queries  1 faileon the chaincode named "mycc" with args ["getMarblesByRange","marble1", "marble201"]
