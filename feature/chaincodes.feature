...
Scenario Outline: FAB-6439: Test chaincode enccc_example.go which uses encshim library extensions.
  ...
  When a user invokes on the chaincode named "mycc" with args ["ENC","PUT","Social-Security-Number","123-45-6789"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\"}"
  And I wait "3" seconds
  When a user queries on the chaincode named "mycc" with args ["ENC","GET", "Social-Security-Number"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\"}"
  Then a user receives a success response of 123-45-6789
  When a user invokes on the chaincode named "mycc" with args ["ENC","PUT","Tax-Id","1234-012"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\",\\"IV\\":\\"<IV>\\"}"
  And I wait "3" seconds
  When a user queries on the chaincode named "mycc" with args ["ENC","GET","Tax-Id"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\",\\"IV\\":\\"<IV>\\"}"
  Then a user receives a response containing 1234-012
  When a user invokes on the chaincode named "mycc" with args ["SIG","PUT","Passport-Number","M9037"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\",\\"SIGKEY\\":\\"<SIGKEY>\\"}"
  And I wait "3" seconds
  When a user queries on the chaincode named "mycc" with args ["SIG","GET","Passport-Number"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\",\\"SIGKEY\\":\\"<SIGKEY>\\"}"
  Then a user receives a response containing M9037
  When a user invokes on the chaincode named "mycc" with args ["ENC","PUT","WellsFargo-Savings-Account","09675879"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\"}"
  And I wait "3" seconds
  When a user invokes on the chaincode named "mycc" with args ["ENC","PUT","BankOfAmerica-Savings-Account","08123456"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\"}"
  And I wait "3" seconds
...
@daily
Scenario Outline: FAB-6439-1: Test chaincode enccc_example.go negative scenario, passing in incorrect ENC KEY
  ...
  When a user invokes on the chaincode named "mycc" with args ["ENC","PUT","Social-Security-Number","123-45-6789"] and transient args "{\\"ENCKEY\\":\\"<ENCKEY>\\"}"
  And I wait "3" seconds
  Then a user receives an error response of Error: Error parsing transient string: illegal base64 data at input byte 43 - <nil>

Examples:
    | type  | waitTime |         ENCKEY                                |            IV              |                           SIGKEY                                                                                                                                                                                                                                                                                 |
    | kafka |    60    |   L6P9jLWR6d6E1KdGJBsUpzEm5QS6uVlS4onsteB+Ka= |  +4DANc5uYLTnsH6Yy7v32g==  |  |

@daily
Scenario Outline: FAB-6439-2: Test chaincode enccc_example.go negative scenario, passing in incorrect IV KEY
...
@daily
Scenario Outline: FAB-6439-3: Test chaincode enccc_example.go negative scenario, passing in incorrect SIGKEY 
...
Examples:
    | type  | waitTime |         ENCKEY                                |      SIGKEY                                                                                                                                                                                                                                                                                                       |
    | kafka |    60    |   L6P9jLWR6d6E1KdGJBsUpzEm5QS6uVlS4onsteB+KaQ= |   |
