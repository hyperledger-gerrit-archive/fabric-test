...
let channel = client.newChannel(chaincode.channelId);
	...
	// return common.getRegisteredUsers(client, user, user.split('@')[1], network_config_path['networkID'], network_config_path['network-config'][userOrg]['mspid']).then((user) => {
	return common.getRegisteredUsers(client, user, user.split('@')[1], network_config_path['networkID'], network_config[userOrg]['mspid']).then((user) => {
		...
		if (response_payloads) {
			...
			if (response_payloads.toString().indexOf('Error') !== -1) {
				console.info(JSON.stringify(["error", response_payloads.toString() + '\n']));
...
// query('User1@org1.example.com', 'Org1ExampleCom', {"args": ["a"], "fcn":"query", "channelId": "behavesystest", "chaincodeId": "mycc"}, ["peer0.org1.example.com"], "/opt/gopath/src/github.com/hyperledger/fabric-test/feature/configs/3ea89a44b03211e79e510214683e8447/network-config.json");
