...
var query = function(user, userOrg, chaincode, peer, network_config_path) {
    ...
    try {
        ...
        network_config = network_config_path['nodes']['network-config'];
    } catch(err) {
        console.error(err);
        return {"error": err};
    }
    ...
    common.getRegisteredUsers(client, user, user.split('@')[1], network_config_path['nodes']['networkID'], network_config[userOrg]['mspid'])
	   .then((tlsInfo) => {
                    client.setTlsClientCertAndKey(tlsInfo.certificate, tlsInfo.key);
                    return Client.newDefaultKeyValueStore({path: common.getKeyStoreForOrg(userOrg)});
            }).then((store) => {
                    client.setStateStore(store);
                    return common.getRegisteredUsers(client, user, user.split('@')[1], network_config_path['nodes']['networkID'], network_config[userOrg]['mspid'])
            }).then((admin) => {
                    the_user = admin;
                    tx_id = client.newTransactionID();
                    common.setupPeers(peer, channel, userOrg, client, network_config, network_config_path['nodes']['tls']);
