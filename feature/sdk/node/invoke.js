...
function invoke(username, org, cc, inputPeerNames, orderer, network_config_path, options) {
    ...
    if(opts && opts.transaction && opts.transaction.localeCompare("true") === 0){
        ...
        return _invoke(username, org, chaincode, inputPeerNames, orderer, network_config)
    }
};
 ...
 * @param {[String]} inputPeerNames the peers
 * @param {String} orderer the orderer
 * @param {JSON} network_config the network configuration
 */
function _invoke(username, org, chaincode, inputPeerNames, orderer, network_config) {
    let peerNames = [inputPeerNames];
    let channel;
...
async function _submitTransaction(org, chaincode, network_config){
    ...
    try {
        ...
        const result = await contract.executeTransaction(...args);
