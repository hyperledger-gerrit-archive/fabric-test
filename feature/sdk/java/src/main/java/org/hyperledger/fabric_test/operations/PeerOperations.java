...
//                peerInfo.getAsJsonObject("grpcOptions").get("grpc.keepalive_time_ms").getAsInt());
        peerProperties.put("grpc.NettyChannelBuilderOption.keepAliveTimeout", new Object[] {8L, TimeUnit.SECONDS});
//                peerInfo.getAsJsonObject("grpcOptions").get("grpc.keepalive_timeout_ms").getAsInt());
...
//        System.out.println(caInfo.getCAName());
        ...
        // Enroll using Fabric-CA
//        //Path certPath = Paths.get(configPath,"peerOrganizations", mspId, "users", enrollId, "tls", String.format("%s.pem", userName);
//        HFCAClient caClient = HFCAClient.createNewInstance(caInfo);
//        caClient.setCryptoSuite(CryptoSuite.Factory.getCryptoSuite());
//        System.out.println(caInfo.getUrl());

//        EnrollmentRequest enrollmentRequest = new EnrollmentRequest();
//        //enrollmentRequest.addHost(caInfo.getUrl());
//        //enrollmentRequest.addHost(caUrl);
//        caUrl = caUrl.replace("localhost", caName);
...
//        return caClient.enroll(enrollId, password, enrollmentRequest);
    ...
    private static PrivateKey getPrivateKeyFromPEMString(String privatePem) throws IOException, NoSuchAlgorithmException, InvalidKeySpecException, IllegalAccessException, InstantiationException, ClassNotFoundException, CryptoException, InvalidArgumentException, CertificateException {
        //String privateStr = privatePem.replace("\n", "");
        //byte[] privateBytes = privateStr.getBytes();
